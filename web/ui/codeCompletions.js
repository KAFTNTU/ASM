...
function arithmeticForms(mnemonic) {
    return [
...
function logicalForms(mnemonic, carryForms) {
    const forms = [
        [`${mnemonic} A,#0x00`, "A, immediate"],
        [`${mnemonic} A,direct`, "A, direct"],
        [`${mnemonic} A,@R0`, "A, indirect"],
    ...
    if (carryForms) {
        forms.push([`${mnemonic} C,bit`, "carry, bit"], [`${mnemonic} C,/bit`, "carry, complemented bit"]);
    }
    return forms;
}
function unaryForms(mnemonic, operands) {
    return operands.map((operand) => [`${mnemonic} ${operand}`, `${operand} addressing`]);
}
export const ASM_MNEMONICS = new Set(Object.keys(ASM_FORMS).map((name) => name.toLowerCase()));
const ASM_DIRECTIVE_SPECS = [
...
export const ASM_DIRECTIVES = new Set(ASM_DIRECTIVE_SPECS.flatMap(([template, , terms]) => [directiveName(template), ...(terms ?? [])])
...
`.trim().split(/\s+/));
export const C_TYPE_NAMES = new Set(`
...
`.trim().split(/\s+/));
export const C_MEMORY_QUALIFIERS = new Set(`
...
const HARDWARE_COMPLETIONS = HARDWARE_SPECS.map(([mode, trigger, label, description, insertText, searchTerms]) => ({
    mode,
    trigger,
    label,
    description,
...
function cSnippet(trigger, description, insertText, searchTerms = [], category = trigger.startsWith("#") ? "Preprocessor" : "C snippet", label = insertText.split("\n", 1)[0]) {
    return {
        mode: "c",
        trigger,
        label,
...
function directiveName(template) {
    const words = template.trim().split(/\s+/);
    return /^(?:NAME|FLAG)$/i.test(words[0]) ? words[1] : words[0];
}
function buildAsmCompletions() {
    const instructions = Object.entries(ASM_FORMS).flatMap(([mnemonic, forms]) => forms.map(([template, addressing]) => ({
        mode: "asm",
        trigger: mnemonic,
        label: template,
...
function buildCCompletions() {
    const words = new Set([...C_KEYWORDS, ...C_TYPE_NAMES, ...C_MEMORY_QUALIFIERS]);
    const keywordItems = Array.from(words, (word) => ({
        mode: "c",
        trigger: word,
        label: word,
    ...
    const operators = C_OPERATOR_SPECS.map(([symbol, description, searchTerms]) => ({
        mode: "c",
...
function buildAduc841Completions() {
    const sfrs = Object.entries(ADUC841_SFR).map(([name, address]) => ({
        mode: "both",
        trigger: name,
        label: name.toUpperCase(),
    ...
    const bits = Object.entries(ADUC841_BITS).flatMap(([name, address]) => {
...
function buildInterruptCompletions() {
    return ADUC841_INTERRUPTS.map(({ number, vector, source, description }) => ({
        mode: "c",
 ...
 */
export const CODE_COMPLETIONS = Object.freeze([
    ...buildAsmCompletions(),
    ...buildCCompletions(),
...
export const ASM_HIGHLIGHT_SYMBOLS = new Set([
    ...CORE_ASM_SYMBOLS.map((name) => name.replace(/^@/, "")),
    ...ADUC841_SFR_NAMES,
    ...ADUC841_BIT_NAMES,
    ...ASM_NUMBERED_BIT_NAMES,
]);
export const C_HIGHLIGHT_SYMBOLS = new Set([
    ...ADUC841_SFR_NAMES,
    ...ADUC841_BIT_NAMES,
]);
export function getCompletionContext(source, cursor, mode) {
...
export function getCodeCompletions(first, second, third, limit = 120) {
    // Prefix-only overload retained for compiler scripts and non-editor callers.
    if (typeof third === "string") {
        return rankCompletions(second, first, third, limit);
    }
    return getCodeCompletionResult(first, second, third, limit);
}
export function getCodeCompletionResult(source, mode, cursor, limit = 120) {
...
export function collectDynamicCompletions(source, mode) {
    return mode === "asm" ? collectAsmSymbols(source) : collectCSymbols(source);
}
function collectAsmSymbols(source) {
    const symbols = [];
    let macroDepth = 0;
    for (const [index, rawLine] of source.split(/\r?\n/).entries()) {
        ...
        if (equate)
            symbols.push(dynamicSymbol("asm", equate[1], equate[2].toUpperCase(), index + 1));
...
function collectCSymbols(source) {
    const symbols = [];
    ...
    for (const match of clean.matchAll(declarationRe)) {
        ...
        for (const declarator of splitTopLevelCsv(match[1])) {
            ...
            if (name)
                symbols.push(dynamicSymbol("c", name, "Variable", line));
        }
    }
    ...
    for (const match of clean.matchAll(forDeclarationRe)) {
        ...
        for (const declarator of splitTopLevelCsv(match[1])) {
            const name = declaratorName(declarator);
            if (name)
                symbols.push(dynamicSymbol("c", name, "Loop variable", line));
        }
    }
    return uniqueDynamic(symbols);
}
function collectSupportedTypedefs(source) {
    ...
    for (let pass = 0; pass <= candidates.length; pass += 1) {
        ...
        for (const candidate of candidates) {
            if (aliases.has(candidate.name) || !isSupportedTypedefSpecifier(candidate.specifier, aliases))
                continue;
...
function isSupportedTypedefSpecifier(specifier, aliases) {
    if (/[*()[\]{}]/.test(specifier))
        return false;
    if (/^struct\s+[A-Za-z_]\w*$/.test(specifier))
        return true;
...
function dynamicSymbol(mode, name, kind, line, call = false) {
...
function completionScore(item, rawQuery) {
    ...
    if (trigger === query || label === query)
        return priority;
    if (trigger.startsWith(query))
        return priority + 10;
    if (label.startsWith(query) || insertion.startsWith(query))
        return priority + 12;
    if (terms.some((term) => term.startsWith(query)))
        return priority + 14;
    if (fields.some((field) => field.split(/\s+/).some((word) => word.startsWith(query))))
        return priority + 22;
