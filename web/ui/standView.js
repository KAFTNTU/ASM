...
import { MOTOR_PWM_C_SAMPLE, MOTOR_PWM_SAMPLE } from "./motorPwmSample.js";
import { AUDIO_PCM_ASM_SAMPLE, AUDIO_PCM_C_SAMPLE } from "./audioPcmSample.js";
export function renderStand(params) {
    ...
    autosaveBtn.textContent = "Autosave";
    ...
    const motorPanel = createMotorPanel({
        ...
        getScopeSignal: (source) => {
            const p0 = board.readPort("P0");
            const p2 = board.readPort("P2");
            const digitalDuty = (value) => popcount8(value) / 8;
            const audio = board.extraDevices.audio?.getScopeSignal?.() ?? { active: false, duty: 0, frequencyHz: 0 };
            if (source === "general") {
                const motor = board.extraDevices.motor.getTelemetry();
                const joystickX = board.getJoystick().x / 4095;
                const joystickMoved = Math.abs(joystickX - 0.5) > 0.08;
                const sources = [
                    { active: motor.active, duty: motor.duty, frequencyHz: motor.frequencyHz },
                    { active: cpu.isRunning(), duty: digitalDuty(~p0), frequencyHz: 800 },
                    { active: cpu.isRunning(), duty: digitalDuty(~p0), frequencyHz: 120 },
                    { active: cpu.isRunning(), duty: 0.2 + digitalDuty(p0 ^ p2) * 0.4, frequencyHz: 700 },
                    { active: cpu.isRunning() || board.getPressedKeys().length > 0, duty: 0.25, frequencyHz: 250 },
                    { active: cpu.isRunning(), duty: digitalDuty(p0), frequencyHz: 1000 },
                    audio,
                    { active: joystickMoved, duty: joystickX, frequencyHz: 35, analogVoltage: joystickX * 5 },
                ];
                const activeSources = sources.filter((item) => item.active);
                if (!activeSources.length) {
                    return { active: false, duty: 0, frequencyHz: 0 };
                }
                const averageDuty = activeSources.length
                    ? activeSources.reduce((sum, item) => sum + item.duty, 0) / activeSources.length
                    : 0;
                return {
                    active: true,
                    duty: averageDuty,
                    frequencyHz: activeSources.reduce((max, item) => Math.max(max, item.frequencyHz), 0),
                };
            }
            if (source === "audio")
                return audio;
            if (source === "joystick") {
                return { active: true, duty: 1, frequencyHz: 0, analogVoltage: (board.getJoystick().x / 4095) * 5 };
            }
            if (source === "sevenSeg")
                return { active: cpu.isRunning(), duty: digitalDuty(~p0), frequencyHz: 800 };
            if (source === "ledBar")
                return { active: cpu.isRunning(), duty: digitalDuty(~p0), frequencyHz: 120 };
            if (source === "matrix")
                return { active: cpu.isRunning(), duty: 0.2 + digitalDuty(p0 ^ p2) * 0.4, frequencyHz: 700 };
            if (source === "keypad")
                return { active: cpu.isRunning() || board.getPressedKeys().length > 0, duty: 0.25, frequencyHz: 250 };
            if (source === "lcd")
                return { active: cpu.isRunning(), duty: digitalDuty(p0), frequencyHz: 1000 };
            const motor = board.extraDevices.motor.getTelemetry();
            return { active: motor.active, duty: motor.duty, frequencyHz: motor.frequencyHz };
        },
        onLoadDemo: () => {
            sourceMode = "asm";
            modeSelect.value = "asm";
            modeTag.textContent = "ASM";
            editor.value = MOTOR_PWM_SAMPLE;
            programLoaded = false;
            updateLineNumbers();
        ...
        onLoadAudioAsm: () => {
            ...
            modeSelect.value = "asm";
            modeTag.textContent = "ASM";
            editor.value = AUDIO_PCM_ASM_SAMPLE;
            programLoaded = false;
            updateLineNumbers();
            compileAndRender(true);
        },
        onLoadAudioC: () => {
            sourceMode = "c";
            modeSelect.value = "c";
            modeTag.textContent = "C";
            editor.value = AUDIO_PCM_C_SAMPLE;
            programLoaded = false;
            updateLineNumbers();
            compileAndRender(true);
        },
    ...
    function updateAutocomplete() {
        ...
        autocompleteMatches = CODE_COMPLETIONS
            .filter((item) => item.label !== "END")
            .filter((item) => item.mode === sourceMode || item.mode === "both")
            .filter((item) => item.trigger.toLowerCase().startsWith(q) ||
    ...
    modeSelect.addEventListener("change", () => {
        ...
        if (sourceMode === "c" && previousMode === "asm" && editor.value.trim() === MOTOR_PWM_SAMPLE.trim()) {
            editor.value = MOTOR_PWM_C_SAMPLE;
            updateLineNumbers();
        }
        else if (sourceMode === "asm" && previousMode === "c" && editor.value.trim() === MOTOR_PWM_C_SAMPLE.trim()) {
            editor.value = MOTOR_PWM_SAMPLE;
            updateLineNumbers();
        }
        modeTag.textContent = sourceMode.toUpperCase();
...
function highlightCLines(source) {
    ...
    interrupt using reentrant compact small large alien priority task _task
  `.trim().split(/\s+/));
    const types = new Set(`
    void char short int long float double signed unsigned bit sbit sfr sfr16 __sfr __sbit __sfr16
    data idata bdata xdata pdata far code const uint8_t uint16_t uint32_t int8_t int16_t int32_t bool
  `.trim().split(/\s+/));
    const sfrs = new Set(`
  ...
  `.trim().split(/\s+/));
    ...
    _nop_ nop abs min max joystick_x joystick_y joy_x joy_y keypad_read key_read adc_read read_adc bus_write st_write
  `.trim().split(/\s+/));
    let out = "";
    let i = 0;
    while (i < text.length) {
        ...
        if (ident) {
            ...
            else if (sfrs.has(lower))
                out += `<span class="tok-reg">${escapeHtml(word)}</span>`;
            else if (builtins.has(lower) || isFn)
...
const CODE_COMPLETIONS = [
    ...
    { "mode": "asm", "trigger": "asm_ready_build_100", "label": "ASM ready build 100", "description": "Full ASM program: LED line loop through ST841 bus", "insertText": "ORG 0x0000\nSTART:\n    MOV SP,#0x2F\n    SETB P3.6\nMAIN:\n    MOV P0,#0xF7\n    MOV P2,#0x07\n    NOP\n    MOV P2,#0x00\n    CALL DELAY\n    SJMP MAIN\n\nDELAY:\n    MOV R5,#0x50\nD1:\n    MOV R6,#0xFF\nD2:\n    DJNZ R6,D2\n    DJNZ R5,D1\n    RET\nEND" },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 1: C Lab2 write + LED",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 2: C Lab3 Static 0x1234",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 3: C Lab4 keypad to 7seg",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 4: C Lab5 stepper half-step",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 5: C Lab6 ADC to LED/static",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_00",
        "label": "РњРµС‚РѕРґРёС‡РєР° 6: C LED 0x00",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_01",
        "label": "РњРµС‚РѕРґРёС‡РєР° 7: C LED 0x01",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_03",
        "label": "РњРµС‚РѕРґРёС‡РєР° 8: C LED 0x03",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_05",
        "label": "РњРµС‚РѕРґРёС‡РєР° 9: C LED 0x05",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_0f",
        "label": "РњРµС‚РѕРґРёС‡РєР° 10: C LED 0x0F",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_33",
        "label": "РњРµС‚РѕРґРёС‡РєР° 11: C LED 0x33",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_55",
        "label": "РњРµС‚РѕРґРёС‡РєР° 12: C LED 0x55",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_aa",
        "label": "РњРµС‚РѕРґРёС‡РєР° 13: C LED 0xAA",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_f0",
        "label": "РњРµС‚РѕРґРёС‡РєР° 14: C LED 0xF0",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_ff",
        "label": "РњРµС‚РѕРґРёС‡РєР° 15: C LED 0xFF",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_7e",
        "label": "РњРµС‚РѕРґРёС‡РєР° 16: C LED 0x7E",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_81",
        "label": "РњРµС‚РѕРґРёС‡РєР° 17: C LED 0x81",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_18",
        "label": "РњРµС‚РѕРґРёС‡РєР° 18: C LED 0x18",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_24",
        "label": "РњРµС‚РѕРґРёС‡РєР° 19: C LED 0x24",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_42",
        "label": "РњРµС‚РѕРґРёС‡РєР° 20: C LED 0x42",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_66",
        "label": "РњРµС‚РѕРґРёС‡РєР° 21: C LED 0x66",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_99",
        "label": "РњРµС‚РѕРґРёС‡РєР° 22: C LED 0x99",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_c3",
        "label": "РњРµС‚РѕРґРёС‡РєР° 23: C LED 0xC3",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_3c",
        "label": "РњРµС‚РѕРґРёС‡РєР° 24: C LED 0x3C",
    ...
    },
    {
        "mode": "c",
        "trigger": "m_c_led_a5",
        "label": "РњРµС‚РѕРґРёС‡РєР° 25: C LED 0xA5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 26: C Static 0x0000",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 27: C Static 0x0001",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 28: C Static 0x0012",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 29: C Static 0x0034",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 30: C Static 0x0101",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 31: C Static 0x0123",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 32: C Static 0x1234",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 33: C Static 0x4321",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 34: C Static 0x5555",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 35: C Static 0xAAAA",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 36: C Static 0x0F0F",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 37: C Static 0xF00D",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 38: C Static 0xBEEF",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 39: C Static 0xCAFE",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 40: C Static 0x2026",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 41: C Static 0x1760",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 42: C Static 0x8410",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 43: C Static 0x8051",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 44: C Static 0xAD0C",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 45: C Static 0xFFFF",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 46: C write addr 1",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 47: C write addr 2",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 48: C write addr 3",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 49: C write addr 4",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 50: C write addr 5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 51: C write addr 6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 52: C write addr 7",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 53: C write addr 8",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 54: C write addr 1",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 55: C write addr 2",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 56: C write addr 3",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 57: C write addr 4",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 58: C write addr 5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 59: C write addr 6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 60: C write addr 7",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 61: C write addr 8",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 62: C write addr 1",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 63: C write addr 2",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 64: C write addr 3",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 65: C write addr 4",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 66: C write addr 5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 67: C write addr 6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 68: C write addr 7",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 69: C write addr 8",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 70: C keypad counter 1",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 71: C keypad counter 2",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 72: C keypad counter 3",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 73: C keypad counter 4",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 74: C keypad counter 5",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 75: C keypad counter 6",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 76: C keypad counter 7",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 77: C keypad counter 8",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 78: C keypad counter 9",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 79: C keypad counter 10",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 80: C keypad counter 11",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 81: C keypad counter 12",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 82: C keypad counter 13",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 83: C keypad counter 14",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 84: C keypad counter 15",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 85: C keypad counter 16",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 86: C keypad counter 17",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 87: C keypad counter 18",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 88: C keypad counter 19",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 89: C keypad counter 20",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 90: C keypad counter 21",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 91: C keypad counter 22",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 92: C keypad counter 23",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 93: C keypad counter 24",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 94: C keypad counter 25",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 95: C keypad counter 26",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 96: C keypad counter 27",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 97: C keypad counter 28",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 98: C keypad counter 29",
        "description": "case 10: Counter--",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 99: C keypad counter 30",
        "description": "case 11: Counter++",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 100: C for clear 1",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 101: C for clear 2",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 102: C for clear 3",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 103: C for clear 4",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 104: C for clear 5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 105: C for clear 6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 106: C for clear 7",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 107: C for clear 8",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 108: C for clear 1",
        "description": "for loop write 1..1",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=2; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 109: C for clear 2",
        "description": "for loop write 1..2",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=3; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 110: C for clear 3",
        "description": "for loop write 1..3",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=4; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 111: C for clear 4",
        "description": "for loop write 1..4",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=5; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 112: C for clear 5",
        "description": "for loop write 1..5",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=6; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 113: C for clear 6",
        "description": "for loop write 1..6",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=7; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 114: C for clear 7",
        "description": "for loop write 1..7",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=8; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 115: C for clear 8",
        "description": "for loop write 1..8",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=9; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 116: C for clear 1",
        "description": "for loop write 1..1",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=2; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 117: C for clear 2",
        "description": "for loop write 1..2",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=3; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 118: C for clear 3",
        "description": "for loop write 1..3",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=4; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 119: C for clear 4",
        "description": "for loop write 1..4",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=5; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 120: C for clear 5",
        "description": "for loop write 1..5",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=6; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 121: C for clear 6",
        "description": "for loop write 1..6",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=7; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 122: C for clear 7",
        "description": "for loop write 1..7",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=8; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 123: C for clear 8",
        "description": "for loop write 1..8",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=9; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 124: C for clear 1",
        "description": "for loop write 1..1",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=2; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 125: C for clear 2",
        "description": "for loop write 1..2",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=3; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 126: C for clear 3",
        "description": "for loop write 1..3",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=4; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 127: C for clear 4",
        "description": "for loop write 1..4",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=5; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 128: C for clear 5",
        "description": "for loop write 1..5",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=6; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 129: C for clear 6",
        "description": "for loop write 1..6",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid main() { unsigned char i; for(i=1; i!=7; i++) write(i,0xFF); while(1); }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 130: C ADC ch0 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 131: C ADC ch0 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 132: C ADC ch0 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 133: C ADC ch1 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 134: C ADC ch1 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 135: C ADC ch1 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 136: C ADC ch2 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 137: C ADC ch2 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 138: C ADC ch2 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 139: C ADC ch3 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 140: C ADC ch3 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 141: C ADC ch3 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 142: C ADC ch4 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 143: C ADC ch4 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 144: C ADC ch4 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 145: C ADC ch5 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 146: C ADC ch5 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 147: C ADC ch5 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 148: C ADC ch6 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 149: C ADC ch6 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 150: C ADC ch6 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 151: C ADC ch7 high",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 152: C ADC ch7 low",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 153: C ADC ch7 led",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 154: C Stepper p0 r0 d5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 155: C Stepper p0 r0 d20",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 156: C Stepper p0 r0 d80",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 157: C Stepper p0 r0 d150",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 158: C Stepper p0 r0 d250",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 159: C Stepper p0 r1 d5",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 160: C Stepper p0 r1 d20",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 161: C Stepper p0 r1 d80",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 162: C Stepper p0 r1 d150",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 163: C Stepper p0 r1 d250",
        "description": "stepper pattern [1, 2, 4, 8]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 164: C Stepper p1 r0 d5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 165: C Stepper p1 r0 d20",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 166: C Stepper p1 r0 d80",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 167: C Stepper p1 r0 d150",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 168: C Stepper p1 r0 d250",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 169: C Stepper p1 r1 d5",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 170: C Stepper p1 r1 d20",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 171: C Stepper p1 r1 d80",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 172: C Stepper p1 r1 d150",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 173: C Stepper p1 r1 d250",
        "description": "stepper pattern [1, 3, 2, 6, 4, 12, 8, 9]",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 174: C Stepper p2 r0 d5",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 175: C Stepper p2 r0 d20",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[i]); delay(20); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 176: C Stepper p2 r0 d80",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[i]); delay(80); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 177: C Stepper p2 r0 d150",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[i]); delay(150); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 178: C Stepper p2 r0 d250",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[i]); delay(250); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 179: C Stepper p2 r1 d5",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[4-1-i]); delay(5); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 180: C Stepper p2 r1 d20",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[4-1-i]); delay(20); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 181: C Stepper p2 r1 d80",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[4-1-i]); delay(80); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 182: C Stepper p2 r1 d150",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[4-1-i]); delay(150); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 183: C Stepper p2 r1 d250",
        "description": "stepper pattern [3, 6, 12, 9]",
        "insertText": "#include \"ADUC841.h\"\nvoid write(unsigned char Addr, unsigned char Dat) { WR=1; P0=Dat; P2&=0xF0; P2|=(Addr&0x0F); P2&=0xF0; }\nvoid delay(unsigned int T) { unsigned int i; unsigned char j; for(i=0;i!=T;i++) for(j=0;j!=255;j++); }\nconst unsigned char code StepTab[4] = {0x03,0x06,0x0C,0x09};\nvoid main() { unsigned char i; while(1) { for(i=0;i!=4;i++) { write(8, StepTab[4-1-i]); delay(250); } } }\n"
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 184: C LCD ST841",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 185: C LCD ADUC841",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 186: C LCD P0 DATA",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 187: C LCD P2 ADDR",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 188: C LCD WR P3.6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 189: C LCD KEYPAD",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 190: C LCD ADC CH6",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 191: C LCD STEP MOTOR",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 192: C LCD STATIC 7SEG",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 193: C LCD MATRIX",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 194: C LCD HELLO",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 195: C LCD MICROLAB",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 196: C LCD C MODE",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 197: C LCD KEIL STYLE",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 198: C LCD METHODYCHKA",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 199: C LCD LAB TEST",
    ...
    },
    {
        ...
        "label": "РњРµС‚РѕРґРёС‡РєР° 200: C LCD INPUT OK",
