...
export function renderStand(params) {
    ...
    modeSelect.title = "Тип коду і розширення файлу";
    ...
    autosaveBtn.title = "Зберегти код у браузері одним кліком. Галочка означає, що поточний код уже збережено.";
    ...
    const speedButtons = speedMultipliers.map((speed) => {
        const node = button(`${speed}`, speed === 1 ? "speed active" : "speed");
        node.title = `Швидкість виконання ${speed}`;
        node.addEventListener("click", () => setSpeed(speed));
        speedGroup.appendChild(node);
        return { speed, node };
    });
    toolbar.append(runBtn, resetBtn, stepBtn, fileNameInput, modeSelect, fileMenuWrap, traceBtn, speedGroup);
    ...
    const editorTitleLeft = el("div", { class: "editorTitleLeft" });
    const modeTag = el("div", { class: "editorTag mono" });
    modeTag.textContent = "ASM";
    const editorStatus = el("div", { class: "editorStatusChip mono idle" });
    editorStatus.textContent = "";
    editorTitleLeft.append(modeTag, editorStatus);
    const runtimeBar = el("div", { class: "runtimeBar mono" });
    editorTop.append(editorTitleLeft, runtimeBar);
    editorBox.appendChild(editorTop);
    editorStatus.addEventListener("click", () => {
        const firstError = [...diagnosticLines.entries()].find(([, level]) => level === "error");
        if (firstError)
            goToEditorLine(firstError[0]);
    });
    ...
    function updateAutosaveButton() {
        ...
        if (!autosaveEnabled) {
            autosaveBtn.innerHTML = '<span>Автозбереження</span><span class="autosaveIcon off">✕</span>';
            autosaveBtn.title = "Автозбереження вимкнено. Натисни, щоб увімкнути.";
            autosaveBtn.classList.add("disabled");
            return;
        }
        if (isSaved) {
            autosaveBtn.innerHTML = '<span>Автозбереження</span><span class="autosaveIcon on">✓</span>';
            autosaveBtn.title = "Автозбереження увімкнено: код збережено. Натисни, щоб вимкнути.";
            autosaveBtn.classList.add("saved");
        } else {
            autosaveBtn.innerHTML = '<span>Автозбереження</span><span class="autosaveIcon off">✕</span>';
            autosaveBtn.title = "Є незбережені зміни. Автозбереження збереже їх автоматично або натисни, щоб вимкнути.";
    ...
    function pushUndoState(state) {
        if (!state || undoStack.length && undoStack[undoStack.length - 1].text === state.text)
            return;
        undoStack.push(state);
        if (undoStack.length > 120)
            undoStack.shift();
    }
    function captureEditorState(text = editor.value) {
        return { text, start: editor.selectionStart ?? 0, end: editor.selectionEnd ?? editor.selectionStart ?? 0 };
    }
    function recordEditorHistoryFromInput() {
        if (applyingHistory) {
            lastHistoryValue = editor.value;
            return;
        }
        if (editor.value !== lastHistoryValue) {
            pushUndoState({ text: lastHistoryValue, start: editor.selectionStart ?? 0, end: editor.selectionStart ?? 0 });
            lastHistoryValue = editor.value;
            redoStack = [];
        }
    }
    function restoreEditorState(state) {
        ...
        editor.focus();
        editor.setSelectionRange(Math.min(state.start ?? 0, max), Math.min(state.end ?? state.start ?? 0, max));
    ...
    const splitHandle = el("div", { class: "splitHandle", title: "" });
    ...
    function triggerEditorInputUpdate() {
        editor.dispatchEvent(new Event("input", { bubbles: true }));
    }
    function indentSelection() {
        ...
        if (selected.includes("\n")) {
            ...
            editor.setRangeText(indented, lineStart, end, "end");
            editor.setSelectionRange(start + 2, lineStart + indented.length);
        }
        else {
            editor.setRangeText("  ", start, end, "end");
        }
        triggerEditorInputUpdate();
    }
    function unindentSelection() {
        ...
        editor.setRangeText(unindented, lineStart, end, "end");
        editor.setSelectionRange(Math.max(lineStart, start - 2), lineStart + unindented.length);
        triggerEditorInputUpdate();
    }

    function toggleLineComment() {
        const marker = sourceMode === "c" ? "//" : ";";
        const start = editor.selectionStart ?? 0;
        const end = editor.selectionEnd ?? start;
        const value = editor.value;
        const lineStart = value.lastIndexOf("\n", Math.max(0, start - 1)) + 1;
        ...
        const finalEnd = blockEnd === -1 ? value.length : blockEnd;
        const selected = value.slice(lineStart, finalEnd);
        const lines = selected.split("\n");
        const allCommented = lines.every((line) => line.trim() === "" || line.replace(/^\s*/, "").startsWith(marker));
        const changed = lines.map((line) => {
            if (line.trim() === "") return line;
            if (allCommented) {
                const indent = line.match(/^\s*/)?.[0] || "";
                const rest = line.slice(indent.length);
                return indent + rest.replace(marker, "").replace(/^ /, "");
            }
            return line.replace(/^\s*/, (indent) => indent + marker + " ");
        }).join("\n");
        editor.setRangeText(changed, lineStart, finalEnd, "end");
    ...
    function duplicateLineOrSelection() {
        ...
        const insertAt = lineEndRaw === -1 ? value.length : lineEndRaw + 1;
        const text = lineEndRaw === -1 ? "\n" + line : line + "\n";
        editor.setRangeText(text, insertAt, insertAt, "end");
        editor.setSelectionRange(insertAt + text.length, insertAt + text.length);
        triggerEditorInputUpdate();
    }
    editor.addEventListener("keydown", (event) => {
        ...
        if (event.key === "Tab") {
            ...
            event.preventDefault();
            autocompleteUserSelected = true;
            autocompleteIndex = autocompleteIndex < 0 ? autocompleteMatches.length - 1 : (autocompleteIndex - 1 + autocompleteMatches.length) % autocompleteMatches.length;
            updateAutocompleteActiveClass();
        }
        else if (event.key === "PageDown") {
            event.preventDefault();
            autocompleteUserSelected = true;
            autocompleteIndex = autocompleteIndex < 0 ? 0 : Math.min(autocompleteMatches.length - 1, autocompleteIndex + 8);
            updateAutocompleteActiveClass();
        }
        else if (event.key === "PageUp") {
            event.preventDefault();
            autocompleteUserSelected = true;
            autocompleteIndex = autocompleteIndex < 0 ? 0 : Math.max(0, autocompleteIndex - 8);
            updateAutocompleteActiveClass();
        }
    ...
    traceBtn.addEventListener("click", () => {
        debugOpen = !debugOpen;
        debugModal.classList.toggle("hidden", !debugOpen);
        traceBtn.classList.toggle("active", debugOpen);
    ...
    function compileAndRender(expand = false) {
        ...
        const summary = editor.value.trim() ? (asm.ok ? "ok" : "errors") : "";
        showMessages(asm.diagnostics, summary, expand);
        updateRuntimeBar(asm.ok);
    ...
    function showMessages(list, summary, expand = false) {
        list = list.filter((item) => item.level !== "hint");
        ...
        updateEditorStatus(errors, warnings, summary);
        const makeLine = (text, level = "info", line = null) => {
            const row = document.createElement(line != null ? "button" : "div");
            row.className = `diagOutputLine ${level}${line != null ? " clickable" : ""}`;
            row.textContent = text;
            if (line != null) {
                row.type = "button";
                row.addEventListener("click", () => goToEditorLine(line));
            }
            messagesBody.appendChild(row);
        };
        for (const item of list) {
            const prefix = item.line != null ? `L${item.line}  ` : "";
            const message = item.message || (item.level === "error" ? "Помилка компіляції" : "Повідомлення");
            makeLine(`${prefix}${message}`, item.level || "info", item.line ?? null);
        }
        if (!list.length && summary) {
            makeLine(summary === "errors" ? "Є помилка, але компілятор не повернув текст діагностики." : "OK", summary === "errors" ? "error" : "ok");
        }
        if (expand) {
    ...
    function updateEditorStatus(errors, warnings, summary = "") {
        editorStatus.classList.remove("ok", "error", "idle");
        if (!editor.value.trim() && !summary) {
            editorStatus.textContent = "";
            editorStatus.classList.add("idle");
            return;
        }
        if (errors > 0) {
            editorStatus.textContent = errors === 1 ? "ERROR" : `ERROR ${errors}`;
            editorStatus.classList.add("error");
            editorStatus.title = "Є помилки. Натисни, щоб перейти до першої помилки.";
            return;
        }
        editorStatus.textContent = "OK";
        editorStatus.classList.add("ok");
        editorStatus.title = warnings > 0 ? `OK, warnings: ${warnings}` : "Код без помилок";
    }
    ...
    function updateSpeedButtons() {
        for (const item of speedButtons) {
            const active = item.speed === currentSpeed;
            item.node.textContent = `${item.speed}`;
            item.node.className = `topBtn speed ${active ? "active" : ""}`.trim();
        }
    }
    ...
    function syncExecMarker() {
        ...
        const logicalLines = Math.max(1, editor.value.replace(/\r/g, "").split("\n").length);
...
function buildDiagnosticLineMap(list) {
    ...
    for (const item of list) {
        if (item.line == null)
            continue;
        // Підкреслюємо в редакторі тільки реальні error.
        // Warning/hint лишаються в Output, але не фарбують код у жовтий.
        if (item.level !== "error")
            continue;
