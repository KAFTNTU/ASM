...
export function createLogicEditor(options) {
    ...
    let tool = "select";
    let pendingCustomCircuitId = null;
    let pendingWire = null;
    let wireCursor = null;
    let running = false;
    let open = false;
    let lastClockTime = performance.now();
    let evaluation = evaluateCircuit(project, currentCircuitId, adapter);
    let dragState = null;
    let wireBendDragState = null;
    ...
    let clipboard = null;
    let pasteOffset = 0;
    let wireColorPopover = null;
    let wirePreviewPath = null;
    let hoveredPinGroup = null;
    ...
    for (const button of [runButton, truthButton, verifyButton, chipButton, fitButton])
    ...
    fileButton.addEventListener("click", (event) => openMenu(event.currentTarget, [
        ["Нова схема", newProject],
        ["Відкрити JSON…", () => importInput.click()],
        ["Зберегти JSON", exportProject],
        ["Зберегти у браузері", () => persistProject(true)],
    ]));
    editButton.addEventListener("click", (event) => openMenu(event.currentTarget, [
        ["Скасувати", undo],
        ["Повторити", redo],
    ...
    viewButton.addEventListener("click", (event) => openMenu(event.currentTarget, [
    ...
    async function toggleFullscreen() {
        try {
            if (document.fullscreenElement === modal)
                ...
                flashStatus("Браузер не підтримує повноекранний режим");
        }
        catch {
    ...
    function buildLibraryPanel() {
        ...
        search.addEventListener("input", () => {
            ...
            buildLibraryPanel();
            const next = libraryPanel.querySelector(".logicLibrarySearch input");
            next?.focus();
            if (next)
        ...
        for (const item of COMPONENT_LIBRARY) {
            ...
            if (libraryFilter && !searchable.includes(libraryFilter))
                continue;
        ...
        for (const circuit of Object.values(project.circuits).filter((item) => item.id !== project.rootCircuitId && (!libraryFilter || item.name.toLowerCase().includes(libraryFilter)))) {
            const button = document.createElement("button");
            button.className = "logicLibraryItem";
            button.type = "button";
    ...
    function setTool(next) {
        ...
        clearPinHover();
        for (const [kind, button] of quickToolButtons)
            button.classList.toggle("active", kind === tool);
        svg.classList.toggle("wireMode", tool === "wire");
        svg.classList.toggle("deleteMode", tool === "delete");
        updateStatus();
    }
    function componentToolbarButton(kind, titleText, description) {
        const button = document.createElement("button");
        button.type = "button";
    ...
    function onCanvasPointerDown(event) {
        ...
        const bendHandle = target.closest("[data-wire-handle]");
        const dipBitNode = target.closest("[data-dip-bit]");
        const toggleNode = target.closest("[data-logic-toggle]");
        const pinNode = target.closest("[data-pin]");
        const componentNode = target.closest("[data-component-id]");
    ...
    function onCanvasPointerUp(event) {
        ...
        if (pendingWire && tool === "wire") {
            const dropNode = document.elementFromPoint(event.clientX, event.clientY)?.closest("[data-pin]");
            if (dropNode?.dataset.componentId && dropNode.dataset.pin) {
                ...
                if (endpoint.componentId !== pendingWire.componentId || endpoint.pinId !== pendingWire.pinId)
                    handlePinClick(endpoint);
                if (!pendingWire) {
                    releasePointer(event.pointerId);
                    wireCursor = null;
    ...
    function updateWirePreview() {
        if (!pendingWire || !wireCursor || !wirePreviewPath)
            return;
    ...
    function beginWireBendDrag(wireId, pointIndex) {
        ...
        if (!wire || !fromComponent || !toComponent)
            return false;
        ...
        if (pointIndex + 1 < bends.length)
            segmentCandidates.push([pointIndex, pointIndex + 1]);
        if (pointIndex > 0)
            segmentCandidates.push([pointIndex - 1, pointIndex]);
        let axis = "x";
    ...
    function moveWireBend(point) {
        ...
        for (const index of state.affectedIndices)
            bends[index][state.axis] = point[state.axis];
        wire.bendPoints = bends;
        state.moved = true;
        updateWireDom(wire);
    }
    function finishWireBendDrag() {
        const state = wireBendDragState;
    ...
    function updateWireDom(wire) {
        const group = [...svg.querySelectorAll(".logicWireGroup")]
        ...
        if (!group || !fromComponent || !toComponent)
            return;
        const from = getPinPosition(fromComponent, wire.from.pinId, project);
        const to = getPinPosition(toComponent, wire.to.pinId, project);
        ...
        if (stroke) {
            ...
            if (color)
                ...
                stroke.style.removeProperty("stroke");
        }
        group.querySelectorAll("[data-wire-handle]").forEach((handle) => {
    ...
    function updatePinHover(clientX, clientY) {
        ...
        if (!pendingWire)
            return;
        const node = document.elementFromPoint(clientX, clientY)?.closest(".logicPinGroup");
        if (!node?.dataset.componentId || !node.dataset.pin)
            return;
    ...
    function markPinAsInvalid(endpoint) {
        const node = [...svg.querySelectorAll(".logicPinGroup")].find((candidate) => candidate.dataset.componentId === endpoint.componentId && candidate.dataset.pin === endpoint.pinId);
        if (!node)
            return;
        clearPinHover();
        ...
        hoveredPinGroup = node;
        window.setTimeout(() => {
            if (hoveredPinGroup === node)
                clearPinHover();
        }, 700);
    }
    function beginMarquee(point, additive) {
        if (!additive)
            clearSelection();
    ...
    function snapDraggedComponents() {
        if (!dragState || !preDragProject)
            return;
    ...
    function adjustDraggedWireBends(beforeCircuit, selected, dx, dy) {
        ...
        for (const wire of currentCircuit().wires) {
            ...
            if (!beforeWire?.bendPoints?.length)
                continue;
            ...
            if (fromSelected)
                alignEndpointBend(bends[0], beforeWire.from, beforeCircuit, dx, dy);
            if (toSelected)
                alignEndpointBend(bends[bends.length - 1], beforeWire.to, beforeCircuit, dx, dy);
            wire.bendPoints = bends;
        }
    }
    function alignEndpointBend(point, endpoint, circuit, dx, dy) {
        ...
        if (nearlyEqual(point.x, endpointPoint.x))
            point.x += dx;
        else if (nearlyEqual(point.y, endpointPoint.y))
            point.y += dy;
        else if (Math.abs(point.x - endpointPoint.x) < Math.abs(point.y - endpointPoint.y))
            point.x += dx;
        else
            point.y += dy;
    }
    ...
    function openComponentSubcircuit(component) {
    ...
    function onCanvasDoubleClick(event) {
        if (event.type === "dblclick" && performance.now() - pointerDoubleClickTime < 700)
            return;
        const target = event.target;
        const componentNode = target.closest("[data-component-id]");
        if (componentNode) {
        ...
        if (wireNode) {
            ...
            const wire = currentCircuit().wires.find((item) => item.id === wireId);
            if (wire)
                openWireColorMenu(wire, event.clientX, event.clientY);
            renderCanvas();
        }
    }
    function openWireColorMenu(wire, clientX, clientY) {
        closeWireColorPopover();
        closePropertiesPopover();
    ...
    function closeWireColorPopover() {
        colorPopover.classList.add("hidden");
        colorPopover.innerHTML = "";
        wireColorPopover = null;
    }
    function openPropertiesPopover(clientX, clientY) {
        closeWireColorPopover();
    ...
    function closePropertiesPopover() {
        if (propertiesPopover.classList.contains("hidden"))
            return;
    ...
    function handlePinClick(endpoint) {
        ...
        if (pendingWire.componentId === endpoint.componentId && pendingWire.pinId === endpoint.pinId)
            return;
        const pendingDirection = endpointDirection(pendingWire);
        if (!pendingDirection) {
            pendingWire = null;
        ...
        const duplicate = currentCircuit().wires.some((wire) => wire.from.componentId === from.componentId && wire.from.pinId === from.pinId &&
        ...
        if (!duplicate) {
            ...
            persistProject();
            const existingDrivers = currentCircuit().wires.filter((wire) => wire.to.componentId === to.componentId && wire.to.pinId === to.pinId).length;
            if (existingDrivers > 1)
                flashStatus("Вхід має кілька джерел; суперечливі рівні буде позначено червоним");
        }
        pendingWire = null;
        wirePreviewPath = null;
        clearPinHover();
    ...
    `;
        ...
        for (const wire of circuit.wires)
            wireLayer.appendChild(renderWire(wire));
    ...
    function renderWire(wire) {
        ...
        if (conflict)
            path.setAttribute("style", "stroke: #d74242");
        else if (customColor)
            path.setAttribute("style", `stroke: ${customColor}`);
    ...
    function renderBlockBody(group, component, size) {
        const value = displayComponentValue(component);
        const text = (content, x, y, className = "logicBlockSymbol", anchor = "middle") => {
        ...
        if (running) {
            ...
            if (open && (clocksChanged || now - lastClockTime > 100)) {
                ...
                if (!interactionActive)
                    renderCanvas();
                if (!wireBendDragState && !dragState)
                    updateStatus();
                lastClockTime = now;
            }
            else if (!open) {
    ...
    function showVectorVerifier() {
        ...
        showCustomDialog("Перевірити за тестовими векторами", form, "Перевірити", () => {
            ...
            vectors.input.value.split(/\r?\n/).forEach((raw, index) => {
                ...
                if (match[1].length !== inputs.length)
                    diagnostics.push(`Рядок ${index + 1}: потрібно ${inputs.length} вхідних бітів`);
                if (match[2].length !== outputs.length)
                    diagnostics.push(`Рядок ${index + 1}: потрібно ${outputs.length} вихідних символів`);
                if (match[1].length === inputs.length && match[2].length === outputs.length)
                    parsed.push({ input: match[1], expected: match[2].toUpperCase(), line: index + 1 });
            });
    ...
    function enterCircuit(circuitId) {
        if (!project.circuits[circuitId])
            return;
    ...
    function circuitPath(circuitId) {
        if (circuitId === project.rootCircuitId)
            return [project.circuits[project.rootCircuitId]];
    ...
    function findCircuitPath(fromId, targetId, visited) {
    ...
    function deleteSelection() {
        if (!selectedComponentIds.size && !selectedWireIds.size)
            return;
        snapshot();
        ...
        currentCircuit().wires = currentCircuit().wires.filter((wire) => !wireIds.has(wire.id) && !componentIds.has(wire.from.componentId) && !componentIds.has(wire.to.componentId));
        clearSelection();
        persistProject();
        renderAll();
    }
    ...
    function copySelection(showMessage = true) {
        const componentIds = new Set(selectedComponentIds);
        const wireIds = new Set(selectedWireIds);
        for (const wire of currentCircuit().wires) {
        ...
        if (!components.length && !wires.length) {
            if (showMessage)
                flashStatus("Нічого копіювати");
            return;
        }
        clipboard = { components, wires };
        pasteOffset = 0;
        if (showMessage)
            flashStatus(`Скопійовано: ${components.length} елементів, ${wires.length} дротів`);
    }
    ...
    function pasteSelection() {
        ...
        for (const source of clipboard.wires) {
            ...
            if (!validComponentIds.has(fromId) || !validComponentIds.has(toId))
                continue;
            const copy = JSON.parse(JSON.stringify(source));
    ...
    function removeInvalidWires(component) {
        const valid = new Set(getComponentPins(component, project).map((pin) => pin.id));
        currentCircuit().wires = currentCircuit().wires.filter((wire) => (wire.from.componentId !== component.id || valid.has(wire.from.pinId)) &&
    ...
    function setSingleComponentSelection(componentId) {
        selectedComponentIds = new Set([componentId]);
        selectedWireIds.clear();
        selectedId = componentId;
        selectedWireId = null;
    }
    function setSingleWireSelection(wireId) {
    ...
    function selectComponent(componentId, additive) {
        if (!additive) {
            if (!selectedComponentIds.has(componentId))
                setSingleComponentSelection(componentId);
            else
                selectedId = componentId;
            return;
        }
    ...
    function selectWire(wireId, additive) {
        if (!additive) {
    ...
    function addComponentAtNextClick(component) {
    ...
    function persistProject(immediate = false) {
        ...
        const save = () => {
            ...
            saveTimer = null;
            if (immediate)
                flashStatus("Схему збережено у браузері");
        };
        if (immediate)
            save();
        else
    ...
    function updateStatus() {
        const circuit = currentCircuit();
        const issues = [];
        if (evaluation.conflicts.size)
            issues.push(`${evaluation.conflicts.size} конфліктів`);
        if (evaluation.warnings.length)
            issues.push(`${evaluation.warnings.length} попереджень`);
    ...
    function onKeyDown(event) {
        if (!open || dialogLayer.classList.contains("hidden") === false)
            return;
        const key = event.key.toLowerCase();
        const modifier = event.ctrlKey || event.metaKey;
        const editing = isEditingTarget(event.target);
        if (editing && event.key !== "Escape" && !modifier)
            return;
        if (modifier && key === "z") {
            event.preventDefault();
    ...
    function openMenuAt(clientX, clientY, items) {
        dropdown.innerHTML = "";
    ...
    function showCustomDialog(dialogTitle, content, primaryLabel, onPrimary, showCancel = true, wide = false) {
...
function isComponentTool(tool) {
    return tool !== "select" && tool !== "wire" && tool !== "delete";
}
function toolbarButton(icon, label, title, compact = false) {
    const button = document.createElement("button");
    button.type = "button";
...
function propertyTitle(text) {
    const node = div("logicPropertyTitle");
    node.textContent = text;
    return node;
}
function propertyReadOnly(label, value) {
...
function propertyNumber(label, value, min, max, onChange, step = 1) {
    const wrap = div("logicPropertyField");
    const name = document.createElement("label");
    name.textContent = label;
    const input = document.createElement("input");
...
function normalizeWireColor(value) {
...
function wireRoute(wire, from, to) {
    ...
    for (const target of requested) {
        ...
        if (!nearlyEqual(last.x, target.x) || !nearlyEqual(last.y, target.y))
            points.push({ ...target });
    }
...
function routeIntersectsRect(points, rect) {
    ...
    for (let index = 1; index < points.length; index += 1) {
        ...
        if (nearlyEqual(a.y, b.y)) {
            if (a.y >= rect.y && a.y <= bottom && Math.max(Math.min(a.x, b.x), rect.x) <= Math.min(Math.max(a.x, b.x), right))
                return true;
        }
        else if (nearlyEqual(a.x, b.x)) {
            if (a.x >= rect.x && a.x <= right && Math.max(Math.min(a.y, b.y), rect.y) <= Math.min(Math.max(a.y, b.y), bottom))
                return true;
        }
    }
    return false;
...
function toolbarLabelForKind(kind) {
    const labels = {
        SWITCH: "SW", BUTTON: "BTN", CLOCK: "CLK", CONST0: "0", CONST1: "1", DIP4: "DIP",
