...
function emitExprToA(expr, ctx, line) {
    ...
    if (constValue != null)
        return [`mov a,#${toAsmByte(constValue)}`];
    ...
    if (fieldRef) {
        if (fieldRef.mode === "direct")
            return [`mov a,${toAsmByte(fieldRef.addr)}`];
        const load = [`mov a,${fieldRef.pointerAddr}`];
        if (fieldRef.offset)
            load.push(`add a,#${toAsmByte8(fieldRef.offset)}`);
...
function ensureVar(ctx, name, line) {
...
function variableTarget(variable) {
...
function resolveFieldAccess(expr, ctx) {
    ...
    if (ptr) {
        ...
        if (!ptrTarget || ctx.pointerVars.get(ptrName) !== "ram")
            return null;
...
function resolveAddressExpression(expr, ctx) {
    ...
    if (idx) {
        ...
        if (local && constIndex != null)
            return local.baseAddr + (constIndex % local.length);
    }
    return null;
}
function emitStoreToLValue(lhs, rhs, ctx, line) {
    ...
    if (field) {
        ...
        if (field.offset)
            out.push(`add a,#${toAsmByte8(field.offset)}`);
...
function parseSwitchCases(body, line) {
    const cases = [];
    let defaultBody = "";
