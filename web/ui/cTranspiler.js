...
function transpileStatement(statement, lineOffset, ctx) {
    ...
    if (declaration && !/^for\b/i.test(raw)) {
        ...
        for (const item of declarators) {
            ...
            if (localArray) {
                ...
                if (isFloatArray) {
                    while (byteValues.length < byteLength)
                        byteValues.push(0);
                }
    ...
    if (returnExpr) {
        if (ctx.currentFunction.returnSize >= 4 && ctx.currentFunction.returnCategory === "integer") {
            ...
            if (slot)
                valueCode.push(...copyScalarBytes(ensureVar(ctx, "__long_result", line, 4), slot, 4));
        ...
        if (ctx.currentFunction.returnCategory === "floating") {
            ...
            const slot = ctx.returnSlots.get(ctx.currentFunction.name);
            if (slot)
                valueCode.push(...copyScalarBytes(temporary, slot, 4));
            return { code: [...valueCode, `ljmp ${ctx.returnLabel}`], needDelay: false, needWrite: false };
        }
...
function resolveBitOperand(raw, ctx) {
...
function emitHardwareCommand(raw, ctx, line) {
    const match = /^([A-Za-z_]\w*)\s*\(([\s\S]*)\)$/.exec(raw.trim());
    if (!match)
        return null;
    const name = match[1].toLowerCase();
    if (!HARDWARE_COMMANDS.has(name) || ctx.functions.has(name))
        return null;
...
function isSupportedTypedefTarget(target) {
    ...
    if (/^(?:struct|union)\s+[A-Za-z_]\w*$/i.test(normalized) || /^void$/i.test(normalized))
        return true;
...
function scalarInitializerBytes(raw, size, diagnostics, label, category = "integer") {
    if (raw == null)
...
function parseGlobalScalars(source, functions, codeArrays, structDefs, nextVarAddr, nextXdataAddr, diagnostics) {
    ...
    for (const statement of statements) {
        ...
        if (!raw || /^(?:sbit|sfr|typedef|enum)\b/i.test(raw) || raw.includes("(") || /^(?:struct|union)\s+[A-Za-z_]\w*\s*\{/i.test(raw))
            continue;
        ...
        for (const token of splitCsv(declaration.declarators)) {
            ...
            if (array) {
                ...
                if (!length)
                    length = Math.max(1, initializerCount);
                const total = Math.max(1, length * declaration.size);
                if (isFloatArray)
                    while (initialBytes.length < total)
                        initialBytes.push(0);
            ...
            if (scalar[2] && ((isFloat && floatValue == null) || (!isFloat && value == null)))
            ...
            if (declaration.memorySpace === "xdata") {
                ...
                for (let i = 0; i < declaration.size; i++)
                    initCode.push(`mov ${toAsmByte(addr + i)},#${toAsmByte8(bytes[i] ?? 0)}`);
            }
        }
    }
...
function parseStructDefs(source, diagnostics) {
    ...
    let match;
    while ((match = re.exec(source)))
        rawDefs.set(match[2].toLowerCase(), { body: match[3], kind: match[1].toLowerCase() });
    const defs = new Map();
    ...
    for (let pass = 0; pass <= rawDefs.size && unresolved.size; pass++) {
        ...
        for (const name of [...unresolved]) {
            ...
            progressed = true;
            if (!invalid)
                defs.set(name, { name, fields, size: isUnion ? maxFieldSize : offset, kind: isUnion ? "union" : "struct" });
        }
        if (!progressed)
            break;
    }
...
function copyScalarBytes(source, target, count = target.size) {
    const out = [];
    const bytes = Math.min(count, source.size, target.size);
    for (let index = 0; index < bytes; index++)
        out.push(`mov a,${variableTarget(source, index)}`, `mov ${variableTarget(target, index)},a`);
    return out;
}
function emitLongConstant(target, value) {
    const out = [];
    for (let index = 0; index < 4; index++)
        out.push(`mov ${variableTarget(target, index)},#${toAsmByte8(value >>> (index * 8))}`);
    return out;
}
function emitLongAddSub(target, left, right, subtract) {
    const out = [`mov a,${variableTarget(left)}`, subtract ? `clr c` : `add a,${variableTarget(right)}`];
    if (subtract)
        out.push(`subb a,${variableTarget(right)}`);
...
function emitLongBitwise(target, left, right, op) {
    const opcode = op === "&" ? "anl" : op === "|" ? "orl" : "xrl";
    const out = [];
    for (let index = 0; index < 4; index++)
        out.push(`mov a,${variableTarget(left, index)}`, `${opcode} a,${variableTarget(right, index)}`, `mov ${variableTarget(target, index)},a`);
    return out;
}
function emitLongShift(target, value, amount, left) {
    const out = copyScalarBytes(value, target, 4);
    const count = Math.max(0, Math.min(32, amount));
    for (let step = 0; step < count; step++)
        out.push(...emitLongShiftOnce(target, left));
    return out;
}
function emitLongShiftOnce(target, left) {
    const out = ["clr c"];
    if (left) {
        for (let index = 0; index < 4; index++)
            out.push(`mov a,${variableTarget(target, index)}`, "rlc a", `mov ${variableTarget(target, index)},a`);
    }
    else {
        for (let index = 3; index >= 0; index--)
            out.push(`mov a,${variableTarget(target, index)}`, "rrc a", `mov ${variableTarget(target, index)},a`);
    }
    return out;
}
function emitLongVariableShift(target, value, amountExpr, left, ctx, line) {
...
function emitLongNegateInPlace(value) {
    const out = [];
    for (let index = 0; index < 4; index++)
        out.push(`mov a,${variableTarget(value, index)}`, "cpl a", `mov ${variableTarget(value, index)},a`);
    out.push("mov a,#0x1", `add a,${variableTarget(value)}`, `mov ${variableTarget(value)},a`);
    for (let index = 1; index < 4; index++)
        out.push(`mov a,${variableTarget(value, index)}`, "addc a,#0x0", `mov ${variableTarget(value, index)},a`);
    return out;
}
/** Emit restoring 32-bit division, with optional signed two's-complement semantics. */
function emitLongDivMod(target, dividend, divisor, remainderMode, signedOperation, ctx, line) {
    ...
    if (signedOperation) {
        ...
        out.push(`mov a,${variableTarget(dividend, 3)}`, "anl a,#0x80", `jz ${dividendPositive}`, `mov ${variableTarget(dividendNegative)},#0x1`, ...emitLongNegateInPlace(dividend), `sjmp ${dividendSignDone}`, `${dividendPositive}:`, `mov ${variableTarget(dividendNegative)},#0x0`, `${dividendSignDone}:`, `mov a,${variableTarget(divisor, 3)}`, "anl a,#0x80", `jz ${divisorPositive}`, `mov ${variableTarget(divisorNegative)},#0x1`, ...emitLongNegateInPlace(divisor), `sjmp ${divisorSignDone}`, `${divisorPositive}:`, `mov ${variableTarget(divisorNegative)},#0x0`, `${divisorSignDone}:`);
    }
    out.push(...emitLongConstant(target, 0), ...emitLongConstant(remainder, 0), `mov ${variableTarget(counter)},#0x20`, `${loop}:`, "clr c");
    for (let index = 0; index < 4; index++)
        out.push(`mov a,${variableTarget(dividend, index)}`, "rlc a", `mov ${variableTarget(dividend, index)},a`);
    // Shift the partial remainder left and bring in the dividend bit.
    for (let index = 0; index < 4; index++)
        out.push(`mov a,${variableTarget(remainder, index)}`, "rlc a", `mov ${variableTarget(remainder, index)},a`);
    ...
    for (let index = 1; index < 4; index++)
        out.push(`mov a,${variableTarget(remainder, index)}`, `subb a,${variableTarget(divisor, index)}`, `mov ${variableTarget(candidate, index)},a`);
    out.push(`jc ${noSubtract}`);
    for (let index = 0; index < 4; index++)
        out.push(`mov a,${variableTarget(candidate, index)}`, `mov ${variableTarget(remainder, index)},a`);
    ...
    else if (remainderMode)
        out.push(...copyScalarBytes(remainder, target, 4));
    return out;
}
function isLongExpression(expression, ctx) {
    const text = trimOuter(expression.trim());
    if (/(?:0[xX][0-9a-fA-F]+|0[bB][01]+|0[oO][0-7]+|\d+)[lL](?:[uU])?$/.test(text) || /\b(?:uint32_t|int32_t)\b/i.test(text))
        return true;
...
function isSignedLongExpression(expression, ctx) {
    ...
    const variable = resolveVariable(text.toLowerCase(), ctx) ?? resolveXdataVariable(text.toLowerCase(), ctx);
    if (variable)
        return variable.size >= 4 ? variable.signed !== false : true;
    const call = /^([A-Za-z_]\w*)\s*\(/.exec(text);
    if (call) {
        const fn = ctx.functions.get(call[1].toLowerCase());
        if (fn?.returnSize && fn.returnSize >= 4)
            return fn.returnSigned !== false;
    }
    if (/\b(?:uint32_t|unsigned\s+long)\b/i.test(text) || /(?:[uUlL]{2}|[uU][lL]|[lL][uU])$/.test(text))
        return false;
    return true;
}
function referencesLongValue(expression, ctx) {
    if (isLongExpression(expression, ctx))
        return true;
    for (const token of expression.match(/[A-Za-z_]\w*/g) ?? []) {
        ...
        if (variable?.category !== "floating" && (variable?.size ?? 0) >= 4)
            return true;
        if ((ctx.functions.get(token.toLowerCase())?.returnSize ?? 0) >= 4)
            return true;
    }
    return false;
}
function emitExprToLong(expr, ctx, line) {
    const result = ensureVar(ctx, "__long_result", line, 4);
    const trimmed = trimOuter(expr.trim());
    const constant = tryEvalConstWide(trimmed);
    if (constant != null)
        return emitLongConstant(result, constant);
    ...
    if (source) {
        ...
        if (source.size < 4) {
            ...
            if (source.signed)
                out.push(`jnb acc.7,${fill}`, ...Array.from({ length: 4 - source.size }, (_, index) => `mov ${variableTarget(result, source.size + index)},#0xff`), `sjmp ${done}`, `${fill}:`);
    ...
    if (shift) {
        ...
        if (amount == null)
            return [...code, ...emitLongVariableShift(result, value, shift.right, shift.op === "<<", ctx, line)];
    ...
    for (const ops of [["|"], ["^"], ["&"], ["+", "-"], ["/", "%"], ["*"]]) {
        ...
        if (found.op === "+" || found.op === "-")
            return [...code, ...emitLongAddSub(result, left, right, found.op === "-")];
        if (found.op === "|" || found.op === "&" || found.op === "^")
            return [...code, ...emitLongBitwise(result, left, right, found.op)];
        if (found.op === "/" || found.op === "%") {
...
function isFloatExpression(expression, ctx) {
    const text = trimOuter(expression.trim());
    if (/(?:\.\d|\d[eE][+\-]?\d)[fF]?\b/.test(text) || /\bfloat\b/i.test(text))
        return true;
    const variable = resolveVariable(text.toLowerCase(), ctx) ?? resolveXdataVariable(text.toLowerCase(), ctx);
    if (variable?.category === "floating")
        return true;
...
function allocateFloatParts(ctx, prefix, line) {
...
function emitFloatUnpack(source, parts, ctx) {
...
function emitFloatShiftRight(parts, count, ctx, line) {
...
function emitFloatShiftLeftOnce(mantissa) {
    return [
        "clr c",
...
function emitFloatNormalizeMantissa(mantissa, exponent, ctx) {
...
function emitFloatPack(target, sign, exponent, mantissa, ctx) {
...
function emitFloatZero(target) {
    return Array.from({ length: 4 }, (_, index) => `mov ${variableTarget(target, index)},#0x0`);
}
function emitFloatMantissaAdd(target, right) {
    const out = [`mov a,${variableTarget(target, 0)}`, "clr c", `add a,${variableTarget(right, 0)}`, `mov ${variableTarget(target, 0)},a`];
    for (let index = 1; index < 3; index++)
        out.push(`mov a,${variableTarget(target, index)}`, `addc a,${variableTarget(right, index)}`, `mov ${variableTarget(target, index)},a`);
    return out;
}
function emitFloatMantissaSub(target, right) {
    const out = [`mov a,${variableTarget(target, 0)}`, "clr c", `subb a,${variableTarget(right, 0)}`, `mov ${variableTarget(target, 0)},a`];
    for (let index = 1; index < 3; index++)
        out.push(`mov a,${variableTarget(target, index)}`, `subb a,${variableTarget(right, index)}`, `mov ${variableTarget(target, index)},a`);
    return out;
}
function emitFloatAddSub(target, left, right, subtract, ctx, line) {
    ...
    const out = [...emitFloatUnpack(left, a, ctx), ...emitFloatUnpack(right, b, ctx)];
    if (subtract)
        out.push(`mov a,${variableTarget(b.sign)}`, "xrl a,#0x1", `mov ${variableTarget(b.sign)},a`);
...
function emitFloatMul(target, left, right, ctx, line) {
    ...
    for (let index = 0; index < 6; index++)
        out.push(`mov ${variableTarget(product, index)},#0x0`);
    ...
    for (let index = 3; index < 6; index++)
        out.push(`mov ${variableTarget(multiplicand, index)},#0x0`);
    for (let index = 0; index < 3; index++)
        out.push(`mov a,${variableTarget(b.mantissa, index)}`, `mov ${variableTarget(multiplier, index)},a`);
    ...
    for (let index = 1; index < 6; index++)
        out.push(`mov a,${variableTarget(product, index)}`, `addc a,${variableTarget(multiplicand, index)}`, `mov ${variableTarget(product, index)},a`);
    out.push(`${skipAdd}:`, "clr c");
    for (let index = 0; index < 6; index++)
        out.push(`mov a,${variableTarget(multiplicand, index)}`, "rlc a", `mov ${variableTarget(multiplicand, index)},a`);
    out.push("clr c");
    for (let index = 2; index >= 0; index--)
        out.push(`mov a,${variableTarget(multiplier, index)}`, "rrc a", `mov ${variableTarget(multiplier, index)},a`);
    ...
    for (let index = 5; index >= 0; index--)
        out.push(`mov a,${variableTarget(product, index)}`, "rrc a", `mov ${variableTarget(product, index)},a`);
...
function emitFloatExprToTarget(expr, target, ctx, line) {
    const trimmed = trimOuter(expr.trim());
    const constant = tryEvalFloatConst(trimmed);
    if (constant != null)
        return emitFloatConstant(target, constant);
    const unaryMinus = /^-\s*([\s\S]+)$/.exec(trimmed);
    if (unaryMinus) {
    ...
    if (/^\+\s*/.test(trimmed))
        return emitFloatExprToTarget(trimmed.slice(1), target, ctx, line);
    ...
    if (call && ctx.functions.get(call[1].toLowerCase())?.returnCategory === "floating") {
        ...
        const slot = ctx.returnSlots.get(fn.name);
        if (slot)
            out.push(...copyScalarBytes(slot, target, 4));
        return out;
    }
    for (const ops of [["+", "-"], ["*"]]) {
        const found = findTopLevelBinaryOperator(trimmed, ops);
...
function emitVariableIncDec(variable, op, ctx, line = ctx.currentFunction.lineOffset) {
    if (variable.isConst)
        return [];
...
function emitCompoundToVariable(variable, op, rhs, ctx, line) {
    if (variable.category === "floating") {
        ...
        if (op === "*")
            out.push(...emitFloatMul(variable, left, right, ctx, line));
        else if (op === "+" || op === "-")
            out.push(...emitFloatAddSub(variable, left, right, op === "-", ctx, line));
        else
            ctx.diagnostics.push({ level: "error", line, message: `Unsupported runtime float compound operator ${op}=` });
        return out;
    }
    if (variable.size >= 4) {
...
function emitAssignToVariable(variable, expr, ctx, line) {
    ...
    if (variable.category === "floating")
        return emitFloatExprToTarget(expr, variable, ctx, line);
    if (variable.size >= 4) {
...
function emitLongComparisonFalseJump(leftExpr, op, rightExpr, falseLabel, ctx, line) {
    ...
    if (op === "==" || op === "!=") {
        ...
        for (let index = 3; index >= 0; index--)
            out.push(`mov a,${variableTarget(left, index)}`, `cjne a,${variableTarget(right, index)},${mismatch}`);
        if (op !== "==")
            out.push(`sjmp ${falseLabel}`, `${different}:`);
        return out;
    }
    ...
    for (let index = 3; index >= 0; index--)
        out.push(`mov a,${variableTarget(left, index)}`, `cjne a,${variableTarget(right, index)},${different}`);
    if (op === "<" || op === ">")
        out.push(`sjmp ${falseLabel}`);
    out.push(`${different}:`);
    if (op === "<" || op === "<=")
        out.push(`jc ${lessOrEqual}`, `sjmp ${falseLabel}`, `${lessOrEqual}:`);
    else if (op === ">")
...
function emitFloatComparisonFalseJump(leftExpr, op, rightExpr, falseLabel, ctx, line) {
...
function allocateFunctionReturnSlots(functions, nextVarAddr, diagnostics) {
    const slots = new Map();
    for (const fn of functions.values()) {
        if (fn.returnSize < 4 || (fn.returnCategory !== "integer" && fn.returnCategory !== "floating"))
            continue;
        const addr = nextVarAddr.value;
        nextVarAddr.value += 4;
        if (nextVarAddr.value > 0x80)
...
function emitFunctionCall(name, rawArgs, ctx, line) {
    ...
    for (let i = 0; i < Math.min(argItems.length, fn.params.length); i++) {
        ...
        if (param.category === "floating") {
            ...
            for (let byte = 0; byte < Math.min(4, param.size ?? 4); byte++)
                out.push(`mov ${toAsmByte(base + byte)},${variableTarget(temporary, byte)}`);
        }
        else if ((param.size ?? 1) >= 4) {
            const temporary = ensureVar(ctx, "__long_result", line, 4);
            out.push(...emitExprToLong(arg, ctx, line));
            for (let byte = 0; byte < 4; byte++)
                out.push(`mov a,${variableTarget(temporary, byte)}`, `mov ${toAsmByte(base + byte)},a`);
        }
...
function parseSwitchCases(body, baseLine, diagnostics, selectorSize = 2) {
...
function switchExpressionSize(expression, ctx) {
    ...
    if (wideConstant != null && (wideConstant < 0 || wideConstant > 0xffff || /[lL]/.test(expression)))
        return 4;
...
function resolveDirectStructObject(expr, ctx) {
    const text = trimOuter(expr.trim()).toLowerCase();
    if (!/^[a-z_]\w*$/.test(text))
        return null;
    const value = ctx.structVars.get(text);
    if (!value || value.elementCount !== 1)
        return null;
    const def = ctx.structDefs.get(value.structName);
    if (!def)
        return null;
...
function emitCopyStructObject(target, source, ctx, line) {
    ...
    for (let index = 0; index < target.size; index++) {
        if (source.storage === "xdata")
            out.push(...emitLoadByteFromXdataAddress(source.baseAddr + index));
        else
            out.push(`mov a,${toAsmByte(source.baseAddr + index)}`);
        if (target.storage === "xdata")
            out.push(...emitStoreByteToXdataAddress(target.baseAddr + index));
        else
            out.push(`mov ${toAsmByte(target.baseAddr + index)},a`);
    }
    return out;
}
...
function emitStoreBytesToXdataAddress(address, source, count) {
    const out = [`mov dpl,#${toAsmByte8(address)}`, `mov dph,#${toAsmByte8(address >> 8)}`];
    for (let index = 0; index < count; index++) {
        if (index > 0)
            out.push("inc dptr");
...
function emitLoadBytesToTargetFromXdata(address, target, count) {
    const out = [`mov dpl,#${toAsmByte8(address)}`, `mov dph,#${toAsmByte8(address >> 8)}`];
    for (let index = 0; index < count; index++) {
        if (index > 0)
            out.push("inc dptr");
...
function tryEvalConstWide(raw) {
    ...
    try {
        ...
        if (!Number.isSafeInteger(integer) || integer < -0x80000000 || integer > 0xffffffff)
            return null;
        return integer | 0;
    }
    catch {
