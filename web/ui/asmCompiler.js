...
const MAX_PREPROCESS_STEPS = 500000;
const MAX_DATA_ITEMS = 65536;
export function compileAsm(source) {
    ...
    const parsed = [];
    let address = 0;
    const declareSymbol = (name, line, kind) => {
    ...
    const declareConstant = (name, expression, line, kind) => {
        if (!declareSymbol(name, line, kind))
            return;
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (setMatch) {
            ...
            if (previous && previous.kind !== "SET") {
                ...
                if (!previous)
                    userSymbols.set(lower, { line: lineNo, kind: "SET" });
        ...
        if (mnemonic === "align" || mnemonic === "even") {
            ...
            const fill = mnemonic === "align" && operands.length === 2
                ...
                : 0;
            if (boundary == null || !Number.isInteger(boundary) || boundary < 1 || boundary > 0x10000 ||
...
function classifyCompatibilityDirective(text) {
    ...
    if (/^\$mod/i.test(text)) {
        ...
        if (BUILTIN_ASM_MODELS.has(normalized))
            return { handled: true };
        return {
            handled: true,
    ...
    if (/^(?:\.module|using|name|public)\b/i.test(text))
        return { handled: true };
    return { handled: false };
}
function parseCompatibilityIncludeTarget(raw) {
    let target = raw.trim();
    if (!target)
        return null;
    const wrappers = { "(": ")", "<": ">", '"': '"', "'": "'" };
    ...
    if (closing) {
        if (target[target.length - 1] !== closing)
            return null;
        target = target.slice(1, -1).trim();
    }
    if (!target || /\s/.test(target) || /[()<>"']/.test(target))
        return null;
    return target;
}
function preprocessAsm(source, diagnostics) {
...
function processPreprocessorLines(input, state, context) {
    const output = [];
    const conditionals = [];
    ...
    for (let index = 0; index < input.length && !state.aborted; index++) {
        ...
        if (macroHeader || repeatHeader) {
            ...
            if (!collected.body.length) {
                if (!chargePreprocessorSteps(state, sourceLine.line, count))
                    break;
                continue;
            }
            for (let repeatIndex = 0; repeatIndex < count && !state.aborted; repeatIndex++) {
                if (!chargePreprocessorSteps(state, sourceLine.line))
                    break;
        ...
        if (invocation) {
            ...
            if (invocation.label) {
                ...
                if (!emitPreprocessedLine(output, labelLine, state))
                    break;
            }
        ...
        if (!emitPreprocessedLine(output, sourceLine, state))
            break;
        if (isEndDirective(text)) {
            reportOpenConditionals();
            return { lines: output, exitMacro: false, stopSource: true };
...
function abortPreprocessor(state, line, message) {
    if (state.aborted)
        return;
    state.aborted = true;
    state.diagnostics.push({ level: "error", line, message });
}
function chargePreprocessorSteps(state, line, count = 1) {
    state.steps += count;
    if (state.steps <= MAX_PREPROCESS_STEPS)
        return true;
...
function emitPreprocessedLine(output, line, state) {
...
function parseMacroHeader(text) {
    const match = /^([A-Za-z_.$?][\w.$?]*)\s+macro\b(.*)$/i.exec(text);
    return match ? { name: match[1], parameterText: match[2].trim() } : null;
}
function collectPreprocessorBlock(input, startIndex, initialKind) {
    const body = [];
    const stack = [initialKind];
    for (let index = startIndex + 1; index < input.length; index++) {
        ...
        if (/^endm(?:\s+.*)?$/i.test(text)) {
            stack.pop();
            if (!stack.length)
                return { body, endIndex: index, closed: true };
            body.push(input[index]);
            continue;
        }
...
function parseMacroInvocation(text, macros) {
    const labelMatch = /^([A-Za-z_.$?][\w.$?]*):\s*(.*)$/.exec(text);
...
function instantiateMacro(definition, args, invocation, state) {
    ...
    return body.map((line) => ({
        text: substituteAsmIdentifiers(line.text, (identifier) => localReplacements.get(identifier.toLowerCase()) ?? identifier),
        line: invocation.line,
        trace: [...line.trace, `MACRO ${definition.name} defined at line ${definition.line}`],
    }));
}
function recordPreprocessorSymbols(line, state) {
...
function isPreprocessorDefined(name, state) {
    const lower = name.toLowerCase();
    return state.defined.has(lower) || state.macros.has(lower) || lower in ADUC841_SFR || lower in ADUC841_BITS;
}
function resolvePreprocessorValue(expression, state, resolving = new Set()) {
    return evaluateAsmExpression(expression, (name) => {
        const lower = name.toLowerCase();
        ...
        if (lower in ADUC841_SFR)
            return ADUC841_SFR[lower];
        if (lower in ADUC841_BITS)
            return ADUC841_BITS[lower];
        return null;
    }, (name) => isPreprocessorDefined(name, state));
}
function substituteAsmIdentifiers(text, replace) {
    let output = "";
    let index = 0;
    let quote = "";
    let escaped = false;
...
function scopeLocalLabels(lines, diagnostics) {
    ...
    return lines.map((line) => {
        ...
        const rewritten = substituteAsmIdentifiers(line.text, (identifier, index) => {
            if (!identifier.startsWith("."))
                return identifier;
            if (index === mnemonicStart && dottedDirectives.has(identifier.slice(1).toLowerCase()))
                return identifier;
            ...
            if (index === labelStart || identifier.startsWith("."))
                return `${effectiveScope}${identifier}`;
            return identifier;
        });
...
function decodeDataString(text) {
    const quote = text[0];
...
function splitOperandList(tail) {
    const operands = [];
    let start = 0;
    let depth = 0;
    let quote = "";
...
function parseDupOperand(operand) {
    let quote = "";
    let escaped = false;
    let depth = 0;
    for (let index = 0; index < operand.length;) {
        ...
        if (char === ")") {
            depth -= 1;
            if (depth < 0)
                return { found: true, count: "", body: "", error: "Unexpected ')' in DUP expression." };
            index += 1;
            continue;
        }
        if (/[A-Za-z_.$?]/.test(char)) {
            let end = index + 1;
            while (end < operand.length && /[\w.$?]/.test(operand[end]))
                end += 1;
            ...
            if (depth === 0 && word.toLowerCase() === "dup") {
                ...
                if (!count)
                    return { found: true, count, body: "", error: "DUP requires a repeat count." };
                if (!suffix.startsWith("("))
                    return { found: true, count, body: "", error: "DUP requires a parenthesized value list." };
                let innerQuote = "";
                ...
                if (closing < 0)
                    return { found: true, count, body: "", error: "Unterminated DUP value list." };
...
function expandDataOperands(operands, labels, equ, currentAddress, line, diagnostics) {
    const fail = (message) => {
        diagnostics.push({ level: "error", line, message });
        return null;
    };
    const expandOne = (operand, depth = 0) => {
        if (depth > 32)
            return fail("DUP nesting exceeds 32 levels.");
        ...
        if (!bodyOperands.length)
            return fail("DUP requires at least one value.");
        const expandedBody = [];
        for (const bodyOperand of bodyOperands) {
            ...
            if (expandedBody.length > MAX_DATA_ITEMS)
    ...
    for (const operand of operands) {
        ...
        if (result.length > MAX_DATA_ITEMS)
            return fail(`Data directive expands past ${MAX_DATA_ITEMS} items.`);
    }
    return result;
}
...
function estimateSize(mnemonic, operands, equ) {
    ...
    const isA = (value) => value === "a" || value === "acc";
    const isIndirect = (value) => value === "@r0" || value === "@r1";
    switch (mnemonic) {
        ...
        case "mov":
            ...
            if (!isRegisterToken(arg0) && !isIndirect(arg0) && !isA(arg0) &&
                !isRegisterToken(arg1) && !isIndirect(arg1) && !isA(arg1))
                return 3;
            return 2;
        case "xch":
            if (operands.length !== 2 || !isA(arg0))
                return 0;
            return isRegisterToken(arg1) || isIndirect(arg1) ? 1 : 2;
        ...
        case "xrl":
            ...
            if (arg0 === "c")
                return 2;
            if ((isRegisterToken(arg0) || isIndirect(arg0)) && arg1.startsWith("#"))
                return 4;
...
function encodeInstruction(entry, labels, equ, diagnostics) {
        ...
        for (const operand of entry.operands) {
            ...
            if (stringValue != null) {
                for (const character of stringValue)
                    bytes.push((character.codePointAt(0) ?? 0) & 0xff);
                continue;
            }
    ...
    switch (entry.mnemonic) {
        ...
        case "jnb": {
            ...
            if (rel == null)
                return fail(`${entry.mnemonic.toUpperCase()} target is out of range (-128..127).`);
        ...
        case "jbc": {
            ...
            const rel = relativeOffset(entry.address, 3, target.value);
            if (rel == null)
                return fail("JBC target is out of range (-128..127).");
        ...
        case "cjne": {
            ...
            if (left.type === "a" && right.type === "imm") {
                const rel = relativeOffset(entry.address, 3, target.value);
                if (rel == null)
                    return fail("CJNE target is out of range (-128..127).");
            ...
            if (left.type === "direct" && right.type === "imm") {
                ...
                if (rel == null)
                    return fail("CJNE direct compatibility target is out of range (-128..127).");
...
function parseOperand(operand, labels, equ, hint = "any", currentAddress = 0) {
    ...
    if (lower.startsWith("/")) {
        ...
        if (positive.type === "bit")
            return { type: "nbit", value: positive.value };
    ...
    if (sfrBitMatch) {
        ...
        const baseAddr = resolveValue(baseName, labels, equ, currentAddress) ??
            ADUC841_SFR[baseName];
    ...
    if (lower in ADUC841_SFR) {
        return { type: "direct", value: ADUC841_SFR[lower] };
    }
    if (labels.has(lower)) {
...
function resolveValue(token, labels, equ, currentAddress = 0, resolving = new Set()) {
    return evaluateAsmExpression(token, (name) => {
        const lower = name.toLowerCase();
        if (lower === "$")
            return currentAddress & 0xffff;
        if (labels.has(lower))
            return labels.get(lower) ?? null;
...
function evaluateAsmExpression(raw, resolveName, isDefined) {
    ...
    const parsePrimary = () => {
        ...
        if (token.kind === "number")
            return token.value;
        if (token.kind === "name")
            return suppressedEvaluation ? 0 : resolveName(token.value);
    ...
    const binary = (next, operators, apply) => {
        ...
        while (true) {
            ...
            if (operatorToken?.kind !== "op" || !operators.includes(operatorToken.value))
                break;
    ...
    const parseMul = () => binary(parseUnary, ["*", "/", "%"], (left, operator, right) => {
    ...
    const parseAdd = () => binary(parseMul, ["+", "-"], (left, operator, right) => operator === "+" ? left + right : left - right);
    const parseShift = () => binary(parseAdd, ["<<", ">>"], (left, operator, right) => {
        if (right < 0 || right > 31)
            return null;
        return operator === "<<" ? left * 2 ** right : left >> right;
    });
    const parseRelational = () => binary(parseShift, ["<", "<=", ">", ">="], (left, operator, right) => {
        if (operator === "<")
    ...
    const parseEquality = () => binary(parseRelational, ["=", "==", "!=", "<>"], (left, operator, right) => operator === "=" || operator === "=="
        ? (left === right ? 1 : 0)
        : (left !== right ? 1 : 0));
    const parseAnd = () => binary(parseEquality, ["&"], (left, _operator, right) => left & right);
    const parseXor = () => binary(parseAnd, ["^"], (left, _operator, right) => left ^ right);
    const parseOr = () => binary(parseXor, ["|"], (left, _operator, right) => left | right);
...
function tokenizeAsmExpression(raw) {
    const tokens = [];
    const wordOperators = {
        shl: "<<",
        shr: ">>",
