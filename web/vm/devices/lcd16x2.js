...
export class Lcd16x2 {
    ...
    print(row, col, text) {
        ...
        this.lines[row] = (line.slice(0, c) + t + line.slice(c + t.length)).slice(0, LCD_COLS);
    }
    write(data) {
        const rs = (data & 0x01);
        ...
        if (this.pendingNibble.rs !== rs) {
            this.pushBusLog(`0x08 <- ${hexByte(this.pendingNibble.raw)} + ${hexByte(data)} RS mismatch ${this.pendingNibble.rs}/${rs}`);
        }
        this.pushBusLog(`0x08 <- ${hexByte(this.pendingNibble.raw)} + ${hexByte(data)} = ${hexByte(byte)} RS=${this.pendingNibble.rs}`);
        this.pendingNibble = null;
    ...
    cursorToLineAndColumn(ddramAddress) {
...
function findLineByBases(ddramAddress, bases) {
