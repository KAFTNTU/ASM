...
type OperandHint = "any" | "immediate" | "bit";
...
function estimateSize(mnemonic: string, operands: string[], equ: Map<string, string>): number {
  ...
  switch (mnemonic) {
    ...
    case "xrl":
      ...
      if ((arg0 === "a" || arg0 === "acc") && isRegisterToken(arg1)) return 1;
      ...
      if ((isRegisterToken(arg0) || arg0 === "@r0" || arg0 === "@r1") && arg1.startsWith("#")) return 4;
    ...
    case "cjne":
      ...
      if (isRegisterToken(arg0) || arg0 === "a" || arg0 === "acc" || arg0 === "@r0" || arg0 === "@r1") return 8;
      if (arg1.startsWith("#") && resolveNumber(arg0, equ) != null) return 10;
      return 8;
    case "djnz":
      if (operands.length !== 2) return 0;
  ...
  switch (entry.mnemonic) {
    case "db": {
      ...
      for (const operand of entry.operands) {
        ...
        const stringMatch = /^(?:'([^']*)'|"([^"]*)")$/.exec(text);
        if (stringMatch) {
          for (const ch of stringMatch[1] ?? stringMatch[2] ?? "") bytes.push(ch.charCodeAt(0) & 0xff);
          continue;
        }
    ...
    case "cjne": {
      ...
      if (left.type === "a" && right.type === "imm") {
        ...
        if (rel == null) return [0xb4, right.value, 0x02, 0x80, 0x03, 0x02, (target.value >> 8) & 0xff, target.value & 0xff];
        return [0xb4, right.value, rel];
      }
      if (left.type === "a" && right.type === "direct") {
        const rel = relativeOffset(entry.address, 3, target.value);
        if (rel == null) return [0xb5, right.value, 0x02, 0x80, 0x03, 0x02, (target.value >> 8) & 0xff, target.value & 0xff];
        return [0xb5, right.value, rel];
      }
      if (left.type === "reg" && right.type === "imm") {
        const rel = relativeOffset(entry.address, 3, target.value);
        if (rel == null) return [0xb8 + left.value, right.value, 0x02, 0x80, 0x03, 0x02, (target.value >> 8) & 0xff, target.value & 0xff];
        return [0xb8 + left.value, right.value, rel];
      }
      if (left.type === "indirect" && right.type === "imm") {
        const rel = relativeOffset(entry.address, 3, target.value);
        if (rel == null) return [0xb6 + left.value, right.value, 0x02, 0x80, 0x03, 0x02, (target.value >> 8) & 0xff, target.value & 0xff];
...
function parseOperand(
  ...
  if (/^(?:'[^']*'|"[^"]*")$/.test(raw)) return { type: "imm", value: 0 };
  if (raw.startsWith("#")) {
...
function operandHints(mnemonic: string, operands: string[]): OperandHint[] {
  ...
  if (m === "jb" || m === "jnb" || m === "jbc") return ["bit", "any"];
  if (m === "mov") {
...
function resolveAliasToken(token: string, equ: Map<string, string>, depth = 0): string {
