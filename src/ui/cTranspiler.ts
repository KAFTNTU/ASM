...
import { ADUC841_BITS, ADUC841_INTERRUPT_VECTORS, ADUC841_SFR } from "../mcu/aduc841";
...
const REGISTER_SET = new Set<string>([...Object.keys(ADUC841_SFR), "a"]);

const BUILTIN_SBITS = new Map<string, string>(
  Object.entries(ADUC841_BITS).map(([name, address]) => [name, formatBuiltinBitOperand(name, address)]),
);
...
const C_DECLARATION_RE = new RegExp(
  `^((?:(?:${C_TYPE_QUALIFIER_SOURCE}|${C_SCALAR_TYPE_SOURCE})\\s+)+)([\\s\\S]+)$`,
  "i",
);

type ParsedStatement = { text: string; line: number };
type PointerKind = "ram" | "code" | "xdata" | "pdata" | "far";
type ScalarTypeInfo = {
  size: number;
  signed: boolean;
  category: "integer" | "floating";
...
type CodeArrayVar = { name: string; bytes: number[]; elementSize: number; elementCount: number };
type StructField = {
  name: string;
  offset: number;
  size: number;
...
type StructVar = {
  ...
  structName: string;
  baseAddr: number;
  storage: "ram" | "xdata" | "code";
  elementCount: number;
  isConst?: boolean;
};
type StructAddressTerm = { expression: string; stride: number };
type StructAccess = {
  storage: "ram" | "xdata" | "code";
  size: number;
  signed: boolean;
  ...
  terms: StructAddressTerm[];
};
type Sfr16Info = { low: string; high: string };

type TranspileContext = {
  sbitMap: Map<string, string>;
  sfrMap: Map<string, string>;
  sfr16Map: Map<string, Sfr16Info>;
  ...
  flowStack: Array<{ breakLabel: string; continueLabel?: string }>;
  returnLabel: string;
  currentFunction: FunctionInfo;
  pointerVars: Map<string, PointerKind>;
  pointerElementSizes: Map<string, number>;
  pointerStructTypes: Map<string, string>;
  pointerPointeeConst: Map<string, boolean>;
  localArrays: Map<string, LocalArrayVar>;
  xdataVars: Map<string, XdataVar>;
  globalXdataVars: Map<string, XdataVar>;
  xdataArrays: Map<string, XdataArrayVar>;
...
export function transpileCToAsm(source: string): CTranspileResult {
  ...
  const usedVectors = new Set<number>();
  ...
  for (const fn of orderedFunctions) {
    ...
    if (fn.interruptNumber != null) {
      ...
      if (fn.usingBank != null) {
        ...
        for (let address = 0; address < 8; address++) emitted.push(`push ${toAsmByte(address)}`);
      }
    }
    ...
    if (fn.interruptNumber != null) {
      if (fn.usingBank == null) {
        for (let address = 7; address >= 0; address--) emitted.push(`pop ${toAsmByte(address)}`);
      }
...
function transpileStatement(
  ...
  if (/^break\s*;?$/i.test(raw)) {
    ...
    if (!flow) ctx.diagnostics.push({ level: "error", line, message: "break used outside a loop or switch." });
  ...
  if (/^continue\s*;?$/i.test(raw)) {
    ...
    if (!flow?.continueLabel) ctx.diagnostics.push({ level: "error", line, message: "continue used outside a loop." });
  ...
  if (structDeclaration) {
    if (structDeclaration.isExtern) return { code: [], needDelay: false, needWrite: false };
    ...
    for (const decl of splitCsv(structDeclaration.declarators)) {
      ...
      if (pointerMatch) {
        ...
        if (pointerMatch[2]) out.push(...emitAssignToVariable(variable, pointerMatch[2], ctx, line));
        else for (let byte = 0; byte < variable.size; byte++) out.push(`mov ${variableTarget(variable, byte)},#0x0`);
        continue;
      }
      ...
      if (storage === "xdata") out.push(...emitXdataConstantBytes(baseAddr, bytes));
      else for (let byte = 0; byte < total; byte++) out.push(`mov ${toAsmByte(baseAddr + byte)},#${toAsmByte8(bytes[byte] ?? 0)}`);
    }
  ...
  if (declaration && !/^for\b/i.test(raw)) {
    if (declaration.isExtern) return { code: [], needDelay: false, needWrite: false };
    ...
    for (const item of declarators) {
      ...
      if (localArray) {
        ...
        if (declaration.memorySpace === "xdata") {
          ...
          for (let i = 0; i < byteLength; i++) out.push(`mov ${toAsmByte(baseAddr + i)},#${toAsmByte8(bytes[i] ?? 0)}`);
        }
        continue;
      }
      ...
      if (pointerDecl) {
        ...
        if (pointerDecl[2]) out.push(...emitAssignToVariable(variable, pointerDecl[2], ctx, line));
        continue;
      }
  ...
  if (compound) {
    ...
    if (variable) {
      if (variable.isConst) ctx.diagnostics.push({ level: "error", line, message: `Cannot modify const variable ${compound[1]}.` });
    ...
    if (xdataVariable) {
      if (xdataVariable.isConst) ctx.diagnostics.push({ level: "error", line, message: `Cannot modify const variable ${compound[1]}.` });
...
function preprocessC(source: string, diagnostics: AsmDiagnostic[]): string {
  const macros = new Map<string, string>([["NULL", "0"], ["true", "1"], ["false", "0"]]);
  const functionMacros = new Map<string, FunctionMacro>();
  const kept: string[] = [];
  const conditions: Array<{ parentActive: boolean; branchTaken: boolean; currentActive: boolean; inElse: boolean }> = [];
  let active = true;
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (directiveName === "ifdef" || directiveName === "ifndef" || directiveName === "if") {
      const condition: boolean = !active
        ? false
        : directiveName === "ifdef"
    ...
    if (directiveName === "elif") {
      ...
      if (!frame || frame.inElse) {
        ...
        const condition: boolean = shouldEvaluate
          ? evaluatePreprocessorCondition(tail, macros, functionMacros, diagnostics, index + 1)
          : false;
    ...
    if (directiveName === "endif") {
      ...
      if (!frame) diagnostics.push({ level: "error", line: index + 1, message: "Unexpected #endif." });
  ...
  if (conditions.length) diagnostics.push({ level: "error", message: "Unclosed #if/#ifdef/#ifndef block." });
  let text = kept.join("\n");
...
function evaluatePreprocessorCondition(
  expression: string,
  ...
  const withDefined = expression
    .replace(/\bdefined\s*\(\s*([A-Za-z_]\w*)\s*\)/g, (_whole, name: string) => macros.has(name) || functionMacros.has(name) ? "1" : "0")
    .replace(/\bdefined\s+([A-Za-z_]\w*)/g, (_whole, name: string) => macros.has(name) || functionMacros.has(name) ? "1" : "0");
...
function expandObjectMacros(source: string, macros: Map<string, string>): string {
  let text = source;
...
function expandOneFunctionMacro(
  source: string,
  name: string,
  macro: FunctionMacro,
  diagnostics: AsmDiagnostic[],
...
function extractEnumConstants(source: string, macros: Map<string, string>, diagnostics: AsmDiagnostic[]): string {
  const enumDefinition = /\benum(?:\s+[A-Za-z_]\w*)?\s*\{([\s\S]*?)\}\s*;/gi;
  return source.replace(enumDefinition, (full, body: string, offset: number) => {
    let nextValue = 0;
...
function spliceContinuedLines(source: string): string {
  const lines = source.split("\n");
  for (let index = 0; index < lines.length - 1; index++) {
    ...
    for (let blank = index + 1; blank <= end; blank++) lines[blank] = "";
  }
  return lines.join("\n");
}

function lexicalCodeMask(source: string): boolean[] {
...
function replaceIdentifierOutsideLiterals(source: string, name: string, replacement: string): string {
  const mask = lexicalCodeMask(source);
  const re = new RegExp(`\\b${escapeRegExp(name)}\\b`, "g");
  return source.replace(re, (match: string, offset: number) => mask[offset] ? replacement : match);
}

function replaceTypedefIdentifier(source: string, name: string, replacement: string): string {
  const mask = lexicalCodeMask(source);
  const shadowRanges = collectTypedefShadowRanges(source, name);
  const re = new RegExp(`\\b${escapeRegExp(name)}\\b`, "g");
  return source.replace(re, (match: string, offset: number) => {
    if (!mask[offset]) return match;
    const before = source.slice(0, offset);
    const after = source.slice(offset + match.length);
    if (shadowRanges.some((range) => offset >= range.start && offset < range.end)) return match;
    if (/\b(?:struct|union|enum)\s*$/i.test(before)) return match;
    if (/(?:\.|->)\s*$/.test(before) || /\bgoto\s*$/i.test(before) || /^\s*:/.test(after)) return match;
    if (isInsideAggregateBody(source, offset) && /^\s*(?:[;,:=]|\[)/.test(after)) return match;
    return replacement;
  });
}
...
function collectTypedefShadowRanges(source: string, name: string): SourceRange[] {
  ...
  while ((match = declaration.exec(code))) {
    ...
    if (openBrace >= 0 && isAggregateBodyOpen(code, openBrace)) continue;
    ...
    if (openParen >= 0 && /\bfor\s*$/i.test(code.slice(0, openParen))) {
      ...
      if (closeParen >= 0) {
        ...
        if (end > tokenOffset) ranges.push({ start: tokenOffset, end });
      }
      continue;
    }
    ...
    if (openBrace >= 0) {
      ...
      if (closeBrace >= 0) ranges.push({ start: tokenOffset, end: closeBrace + 1 });
      continue;
    }
...
function findEnclosingCodeDelimiter(source: string, offset: number, open: string, close: string): number {
  const stack: number[] = [];
  for (let index = 0; index < offset; index++) {
    if (source[index] === open) stack.push(index);
    else if (source[index] === close) stack.pop();
  }
  return stack.at(-1) ?? -1;
}

function isAggregateBodyOpen(source: string, openBrace: number): boolean {
...
function isInsideAggregateBody(source: string, offset: number): boolean {
  const mask = lexicalCodeMask(source);
  const code = Array.from(source, (ch, index) => mask[index] || ch === "\n" ? ch : " ").join("");
...
type RawTypedef = { name: string; target: string; line: number };
type TypedefSpan = { start: number; end: number; line: number; text: string };
...
function expandTypedefAliases(source: string, diagnostics: AsmDiagnostic[]): { source: string; aliases: Map<string, string> } {
  ...
  const resolveAlias = (name: string): string | null => {
    ...
    if (invalid.has(name)) return null;
    ...
    if (cycleAt >= 0) {
      ...
      for (const item of cycle) invalid.add(item);
      return null;
    }
    ...
    if (!taggedStruct) {
      ...
      for (const token of tokens) {
        if (!rawAliases.has(token)) continue;
...
function findTypedefSpans(source: string, diagnostics: AsmDiagnostic[]): TypedefSpan[] {
  const mask = lexicalCodeMask(source);
  const spans: TypedefSpan[] = [];
...
function canonicalizeTypedefTarget(text: string): string {
  const normalized = normalizeTypeText(text);
  if (
    /\b(?:signed|unsigned)\b/i.test(normalized) &&
...
function parseSbits(source: string): Map<string, string> {
  const map = new Map<string, string>(BUILTIN_SBITS);
  ...
  while ((m = re.exec(source))) {
    ...
    if (numeric) map.set(name, numeric);
  }
  return map;
}

function parseSfrs(source: string, diagnostics: AsmDiagnostic[]): { sfrMap: Map<string, string>; sfr16Map: Map<string, Sfr16Info> } {
  const sfrMap = new Map<string, string>();
  const sfr16Map = new Map<string, Sfr16Info>();
...
function parseArrays(source: string, diagnostics: AsmDiagnostic[]): Map<string, CodeArrayVar> {
  const map = new Map<string, CodeArrayVar>();
  const re = new RegExp(
    `((?:(?:${C_TYPE_QUALIFIER_SOURCE}|${C_SCALAR_TYPE_SOURCE})\\s+)+)([A-Za-z_]\\w*)\\s*\\[\\s*([^\\]]*)\\s*\\]\\s*=\\s*(\\{[\\s\\S]*?\\}|"(?:\\\\.|[^"])*")\\s*;`,
    "gi",
  );
  let m: RegExpExecArray | null;
  while ((m = re.exec(source))) {
    ...
    if (!header.includes("code")) continue;
    ...
    if (explicitLength != null) {
      if (values.length > explicitLength) diagnostics.push({ level: "error", message: `Too many initializers for ${name}[${explicitLength}].` });
      while (values.length < explicitLength) values.push(0);
    }
    map.set(name, {
      name,
...
function expandElementBytes(values: number[], elementSize: number, elementCount: number): number[] {
  const bytes: number[] = [];
  for (let element = 0; element < elementCount; element++) {
    const value = values[element] ?? 0;
    for (let byte = 0; byte < elementSize; byte++) bytes.push((value >> (8 * byte)) & 0xff);
  }
  return bytes;
}

function decodeCString(token: string): string {
  const body = token.trim().replace(/^"|"$/g, "");
  return body.replace(/\\(x[0-9a-fA-F]{2}|[0-7]{1,3}|n|r|t|0|\\|"|')/g, (_m, esc: string) => {
    if (esc === "n") return "\n";
    if (esc === "r") return "\r";
    if (esc === "t") return "\t";
    if (esc === "0") return "\0";
...
function parseDeclarationText(raw: string): ParsedDeclaration | null {
...
type InitializerNode = string | InitializerNode[];

function parseInitializerNode(raw: string): InitializerNode {
  const text = raw.trim();
...
function flattenStructInitializer(
  node: InitializerNode | undefined,
  def: StructDef,
  defs: Map<string, StructDef>,
  diagnostics: AsmDiagnostic[],
  label: string,
): number[] | null {
  if (node == null) return Array.from({ length: def.size }, () => 0);
...
function flattenStructObjectInitializer(
  ...
  if (!raw) return Array.from({ length: def.size * count }, () => 0);
  const node = parseInitializerNode(raw);
  if (count === 1) return flattenStructInitializer(node, def, defs, diagnostics, label);
  if (!Array.isArray(node)) {
...
function scalarTypeInfo(raw: string): ScalarTypeInfo | null {
...
function memorySpaceFromHeader(header: string): PointerKind | "data" | "idata" | "bdata" {
  if (/\bxdata\b/i.test(header)) return "xdata";
  if (/\bpdata\b/i.test(header)) return "pdata";
  if (/\bfar\b/i.test(header)) return "far";
  if (/\bcode\b/i.test(header)) return "code";
  if (/\bidata\b/i.test(header)) return "idata";
  if (/\bbdata\b/i.test(header)) return "bdata";
  return "data";
}

function pointerKindFromHeader(header: string): PointerKind {
  const space = memorySpaceFromHeader(header);
  return space === "data" || space === "idata" || space === "bdata" ? "ram" : space;
}

function pointerStorageSize(kind: PointerKind): number {
  return kind === "code" || kind === "xdata" || kind === "far" ? 2 : 1;
}

function unsupportedObjectSpace(space: ParsedDeclaration["memorySpace"]): string | null {
  return space === "pdata" || space === "far" ? space : null;
}

function peelLeadingScalarCasts(raw: string): { expression: string; type: ScalarTypeInfo | null } {
  let expression = trimOuter(raw.trim());
  let outerType: ScalarTypeInfo | null = null;
  for (let guard = 0; guard < 8 && expression.startsWith("("); guard++) {
    const close = findMatchingParenLike(expression, 0, "(", ")");
    if (close <= 0 || close >= expression.length - 1) break;
    const header = expression.slice(1, close).trim();
    if (/\b(?:static|extern|register|auto|code|data|idata|xdata|pdata|bdata|far)\b/i.test(header)) break;
    const declaration = parseDeclarationText(`${header} __c_cast_value`);
    if (!declaration || declaration.declarators !== "__c_cast_value") break;
...
function parseGlobalScalars(
  source: string,
  functions: Map<string, FunctionInfo>,
  codeArrays: Map<string, CodeArrayVar>,
  structDefs: Map<string, StructDef>,
  nextVarAddr: { value: number },
  nextXdataAddr: { value: number },
  diagnostics: AsmDiagnostic[],
  ...
  vars: Map<string, ScalarVar>;
  ramArrays: Map<string, LocalArrayVar>;
  xdataVars: Map<string, XdataVar>;
  xdataArrays: Map<string, XdataArrayVar>;
  structVars: Map<string, StructVar>;
  pointerVars: Map<string, PointerKind>;
  ...
  for (const fn of functions.values()) {
    for (let i = fn.rangeStart; i < fn.rangeEnd && i < chars.length; i++) chars[i] = chars[i] === "\n" ? "\n" : " ";
  }
  const masked = chars.join("");
  const statements = extractTopLevelSemicolonStatements(masked);
  const vars = new Map<string, ScalarVar>();
  const ramArrays = new Map<string, LocalArrayVar>();
  const xdataVars = new Map<string, XdataVar>();
  const xdataArrays = new Map<string, XdataArrayVar>();
  const structVars = new Map<string, StructVar>();
  const pointerVars = new Map<string, PointerKind>();
  const pointerElementSizes = new Map<string, number>();
  const pointerStructTypes = new Map<string, string>();
  const pointerPointeeConst = new Map<string, boolean>();
  const pointerInitializers: Array<{ name: string; addr: number; size: number; kind: PointerKind; initializer?: string; structName?: string }> = [];
  ...
  for (const statement of statements) {
    ...
    if (structDeclaration) {
      if (structDeclaration.isExtern) continue;
      ...
      for (const token of splitCsv(structDeclaration.declarators)) {
        ...
        if (pointer) {
          ...
          if (nextVarAddr.value > 0x80) diagnostics.push({ level: "error", message: `Global pointer ${name} exceeds internal RAM 0x30..0x7F.` });
        ...
        if (storage === "ram") {
          ...
          if (nextVarAddr.value > 0x80) diagnostics.push({ level: "error", message: `Global structure ${name} exceeds internal RAM 0x30..0x7F.` });
        }
        ...
        if (storage === "xdata") initCode.push(...emitXdataConstantBytes(baseAddr, bytes));
        else for (let index = 0; index < total; index++) initCode.push(`mov ${toAsmByte(baseAddr + index)},#${toAsmByte8(bytes[index] ?? 0)}`);
      }
      continue;
    }
    ...
    for (const token of splitCsv(declaration.declarators)) {
      ...
      if (array) {
        if (declaration.isCode) continue;
        const unsupportedSpace = unsupportedObjectSpace(declaration.memorySpace);
        if (unsupportedSpace) {
          diagnostics.push({ level: "error", message: `${unsupportedSpace} object storage is not implemented by the browser C51 backend.` });
          continue;
        ...
        if (declaration.memorySpace === "xdata") {
          ...
          nextVarAddr.value += total;
          if (nextVarAddr.value > 0x80) diagnostics.push({ level: "error", message: `Global array ${name} exceeds internal RAM 0x30..0x7F.` });
          ramArrays.set(name, { name, baseAddr, length: total, elementSize: declaration.size, elementCount: length });
          for (let i = 0; i < total; i++) initCode.push(`mov ${toAsmByte(baseAddr + i)},#${toAsmByte8(bytes[i] ?? 0)}`);
        }
        continue;
      }
      const scalar = /^([A-Za-z_]\w*)(?:\s*=\s*([\s\S]+))?$/.exec(part);
  ...
  for (const pointer of pointerInitializers) {
    ...
    if (initializer) {
      ...
      if (numeric != null) {
        ...
        if (indexed) {
          ...
          if (array && index != null) address = array.baseAddr + normalizedArrayOffset(index, array);
          else if (structObject && index != null && structObject.elementCount > 1) {
            const def = structDefs.get(structObject.structName);
            if (def) address = structObject.baseAddr + index * def.size;
          }
        } else {
    ...
    if (pointer.size >= 2) initCode.push(`mov ${toAsmByte(pointer.addr + 1)},#${high}`);
  }
...
function extractTopLevelSemicolonStatements(source: string): string[] {
  const out: string[] = [];
  let start = 0;
  let brace = 0;
  let paren = 0;
...
function parseStructDefs(source: string, diagnostics: AsmDiagnostic[]): Map<string, StructDef> {
    ...
    for (const name of [...unresolved]) {
      ...
      for (const part of splitStatements(body)) {
        ...
        if (nested) {
          ...
          if (!nestedDef) {
            if (rawDefs.has(nested.structName)) blocked = true;
            else {
  ...
  for (const name of unresolved) diagnostics.push({ level: "error", message: `Cannot resolve nested structure layout for struct ${name}.` });
  return defs;
}
...
function extractFunctions(source: string, diagnostics: AsmDiagnostic[]): Map<string, FunctionInfo> {
  ...
  while ((match = re.exec(source))) {
    ...
    if (functions.has(name)) diagnostics.push({ level: "error", message: `Function ${name} is defined more than once.` });
...
function emitPointerIncDec(variable: ScalarVar, op: string, ctx: TranspileContext): string[] {
  const stride = Math.max(1, ctx.pointerElementSizes.get(variable.name) ?? 1);
  if (variable.size < 2) return [`mov a,${variableTarget(variable)}`, `${op === "++" ? "add" : "add"} a,#${toAsmByte8(op === "++" ? stride : -stride)}`, `mov ${variableTarget(variable)},a`];
...
function emitCompoundToVariable(variable: ScalarVar, op: string, rhs: string, ctx: TranspileContext, line: number): string[] {
  if (variable.size >= 2) {
    ...
    for (let byte = 2; byte < variable.size; byte++) out.push(`mov ${variableTarget(variable, byte)},#0x0`);
    return out;
  }
  if (op === "<<" || op === ">>" || op === "*" || op === "/" || op === "%") {
...
function emitAssignToVariable(variable: ScalarVar, expr: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (constant != null) {
    const out: string[] = [];
    for (let i = 0; i < variable.size; i++) out.push(`mov ${variableTarget(variable, i)},#${toAsmByte8(constant >> (8 * i))}`);
    return out;
  }
  ...
  if (sourceVar) {
    ...
    for (let i = 0; i < bytes; i++) out.push(`mov a,${variableTarget(sourceVar, i)}`, `mov ${variableTarget(variable, i)},a`);
    for (let i = bytes; i < variable.size; i++) out.push(`mov ${variableTarget(variable, i)},#0`);
    return out;
  }
  ...
  if (variable.size >= 2) {
    ...
    const out = [...word, `mov ${variableTarget(variable)},a`, `mov ${variableTarget(variable, 1)},b`];
    for (let i = 2; i < variable.size; i++) out.push(`mov ${variableTarget(variable, i)},#0`);
    return out;
  }
...
function emitCompoundToXdataVariable(variable: XdataVar, op: string, rhs: string, ctx: TranspileContext, line: number): string[] {
  return emitAssignToXdataVariable(variable, `${variable.name} ${op} (${rhs})`, ctx, line);
}

function emitXdataVariableIncDec(variable: XdataVar, op: string, ctx: TranspileContext, line: number): string[] {
  return emitAssignToXdataVariable(variable, `${variable.name} ${op === "++" ? "+" : "-"} 1`, ctx, line);
}

function emitPointerAddConstant(variable: ScalarVar, byteDelta: number, ctx: TranspileContext): string[] {
  if (variable.size < 2) return [`mov a,${variableTarget(variable)}`, `add a,#${toAsmByte8(byteDelta)}`, `mov ${variableTarget(variable)},a`];
...
function emitExprToWord(expr: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (sfr16) return [`mov a,${sfr16.low}`, `mov b,${sfr16.high}`];

  const xdataVariable = resolveXdataVariable(trimmed.toLowerCase(), ctx);
  if (xdataVariable) {
    if (xdataVariable.size >= 2) return emitLoadWordFromXdataAddress(xdataVariable.addr);
    const out = emitLoadByteFromXdataAddress(xdataVariable.addr);
    if (!xdataVariable.signed) return [...out, "mov b,#0x0"];
  ...
  if (xdataArrayDirect) return [`mov a,#${toAsmByte8(xdataArrayDirect.baseAddr)}`, `mov b,#${toAsmByte8(xdataArrayDirect.baseAddr >> 8)}`];
  ...
  if (variable) {
    if (variable.size >= 2) return [`mov a,${variableTarget(variable)}`, `mov b,${variableTarget(variable, 1)}`];
    if (variable.signed) {
  ...
  if (wideStructAccess) return emitLoadStructAccess(wideStructAccess, ctx, line, true);
  ...
  if (call && ctx.functions.has(call[1].toLowerCase())) {
    ...
    if (fn.returnSize < 2) out.push("mov b,#0x0");
    return out;
  }
  ...
  if (trimmed.startsWith("+")) return emitExprToWord(trimmed.slice(1), ctx, line);

  const shift = findTopLevelBinaryOperator(trimmed, ["<<", ">>"]);
  if (shift) return emitWordShift(shift.left, shift.op, shift.right, ctx, line);
  ...
  for (const ops of [["|"], ["^"], ["&"], ["+", "-"], ["*", "/", "%"]]) {
    ...
    if (found.op === "*" || found.op === "/" || found.op === "%") {
      ...
      if (found.op !== "*") out.push("mov b,#0x0");
      return out;
    }
  ...
  if (cast.type) {
    ...
    if (cast.type.size !== 1) return emitExprToWord(cast.expression, ctx, line);
    const out = emitExprToA(cast.expression, ctx, line);
    if (!cast.type.signed) return [...out, "mov b,#0x0"];
  ...
  if (bitName) return [...emitExprToA(trimmed, ctx, line), "mov b,#0x0"];

  const target = resolveTarget(trimmed.toLowerCase(), ctx);
  if (target) return [`mov a,${normalizeAccumulatorSource(target)}`, "mov b,#0x0"];
...
function emitRotateWord(expr: string, count: number, direction: "left" | "right", ctx: TranspileContext, line: number): string[] {
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (direction === "left") {
      out.push(
        `mov a,${variableTarget(value)}`, "clr c", "rlc a", `mov ${variableTarget(value)},a`,
        `mov a,${variableTarget(value, 1)}`, "rlc a", `mov ${variableTarget(value, 1)},a`,
        `jnc ${noWrap}`, `orl ${variableTarget(value)},#0x01`, `${noWrap}:`,
      );
    } else {
      out.push(
        `mov a,${variableTarget(value, 1)}`, "clr c", "rrc a", `mov ${variableTarget(value, 1)},a`,
        `mov a,${variableTarget(value)}`, "rrc a", `mov ${variableTarget(value)},a`,
        `jnc ${noWrap}`, `orl ${variableTarget(value, 1)},#0x80`, `${noWrap}:`,
      );
    }
  }
...
function allocateWordTemp(ctx: TranspileContext, prefix: string, line: number): ScalarVar {
  return ensureVar(ctx, `__${prefix}_${ctx.labelCounter.value++}`, line, 2);
}

function emitWordBinary(left: string, op: string, right: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (op === "+" || op === "-") {
    ...
    if (op === "-") out.push("clr c", `subb a,${variableTarget(rhs)}`);
    else out.push(`add a,${variableTarget(rhs)}`);
  ...
  out.push(
    ...
    `${opcode} a,${variableTarget(rhs)}`,
    `mov ${variableTarget(resultLow)},a`,
    `mov a,${variableTarget(lhs, 1)}`,
    `${opcode} a,${variableTarget(rhs, 1)}`,
    "mov b,a",
    `mov a,${variableTarget(resultLow)}`,
  );
  return out;
}

function emitWordShift(left: string, op: string, right: string, ctx: TranspileContext, line: number): string[] {
  ...
  const emitOne = () => {
    if (op === "<<") {
      out.push(
        "clr c",
        `mov a,${variableTarget(value)}`,
        "rlc a",
        `mov ${variableTarget(value)},a`,
        `mov a,${variableTarget(value, 1)}`,
        "rlc a",
        `mov ${variableTarget(value, 1)},a`,
      );
    } else {
      out.push(
        "clr c",
  ...
  if (countConst != null) {
    for (let i = 0; i < Math.min(16, countConst & 0xff); i++) emitOne();
  } else {
...
function emitAssignToSfr16(target: Sfr16Info, expr: string, ctx: TranspileContext, line: number): string[] {
  const unsupportedLiteral = unsupportedArithmeticLiteral(expr);
  if (unsupportedLiteral) {
    ctx.diagnostics.push({ level: "error", line, message: unsupportedLiteral });
    return [];
...
function emitExprToA(expr: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (constValue != null) return [`mov a,#${toAsmByte8(constValue)}`];

  const sfr16 = ctx.sfr16Map.get(trimmed.toLowerCase());
  if (sfr16) return [`mov a,${sfr16.low}`];

  const directXdata = resolveXdataVariable(trimmed.toLowerCase(), ctx);
  if (directXdata) return emitLoadByteFromXdataAddress(directXdata.addr);
  ...
  if (preInc) {
    ...
    if (xdataVariable) return [...emitXdataVariableIncDec(xdataVariable, preInc[1], ctx, line), ...emitLoadByteFromXdataAddress(xdataVariable.addr)];
    if (!variable) {
  ...
  if (assignmentExpr) {
    ...
    if (assignmentExpr[2] === "=") return [...emitAssignToVariable(variable, assignmentExpr[3], ctx, line), `mov a,${variableTarget(variable)}`];
  ...
  if (trimmed.startsWith("*")) {
    ...
    if (ptr && ptrKind === "ram") return [`mov a,${ptr}`, "mov r0,a", "mov a,@r0"];
    if (ptr && ptrKind === "xdata") {
      const variable = resolveVariable(ptrName, ctx);
      if (variable) return [`mov dpl,${variableTarget(variable)}`, `mov dph,${variableTarget(variable, 1)}`, "movx a,@dptr"];
    }
    if (ptr && ptrKind === "code") {
      const variable = resolveVariable(ptrName, ctx);
      if (variable) return [`mov dpl,${variableTarget(variable)}`, `mov dph,${variableTarget(variable, 1)}`, "clr a", "movc a,@a+dptr"];
    }
  }
  ...
  if (structAccess) return emitLoadStructAccess(structAccess, ctx, line, false);

  const xdataArrayDirect = ctx.xdataArrays.get(trimmed.toLowerCase());
  if (xdataArrayDirect) return [`mov a,#${toAsmByte8(xdataArrayDirect.baseAddr)}`];
  const localArrayDirect = ctx.localArrays.get(trimmed.toLowerCase());
  if (localArrayDirect) return [`mov a,#${toAsmByte8(localArrayDirect.baseAddr)}`];

  const arrayMatch = /^([A-Za-z_]\w*)\s*\[\s*([^\[\]]+)\s*\]$/.exec(trimmed);
  if (arrayMatch) {
    const arrayName = arrayMatch[1].toLowerCase();
    const xdataArray = ctx.xdataArrays.get(arrayName);
    if (xdataArray) return emitXdataArrayLoad(xdataArray, arrayMatch[2], false, ctx, line);
    const localArray = ctx.localArrays.get(arrayName);
    if (localArray) {
      const indexConst = tryEvalConst(arrayMatch[2]);
      if (indexConst != null) return [`mov a,${toAsmByte(localArray.baseAddr + normalizedArrayOffset(indexConst, localArray))}`];
    ...
    if (ctx.pointerVars.has(arrayName)) return emitPointerIndexLoad(arrayName, arrayMatch[2], false, ctx, line);
  ...
  if (trimmed.startsWith("!")) return emitBooleanExprToA(trimmed, ctx, line);
  if (trimmed.startsWith("~")) return [...emitExprToA(trimmed.slice(1), ctx, line), "cpl a"];
  if (trimmed.startsWith("-") && !/^-[0-9]/.test(trimmed)) return [...emitExprToA(trimmed.slice(1), ctx, line), "cpl a", "inc a"];
  if (trimmed.startsWith("+")) return emitExprToA(trimmed.slice(1), ctx, line);

  const shift = findTopLevelBinaryOperator(trimmed, ["<<", ">>"]);
  if (shift) return emitShiftToA(shift.left, shift.op, shift.right, ctx, line);

  for (const ops of [["|"], ["^"], ["&"], ["+", "-"], ["*", "/", "%"]]) {
    const found = findTopLevelBinaryOperator(trimmed, ops);
    if (!found) continue;
    if (found.op === "*" || found.op === "/" || found.op === "%") return emitMulDivToA(found.left, found.op, found.right, ctx, line);
  ...
  const target = resolveTarget(trimmed.toLowerCase(), ctx);
  if (target) return [`mov a,${normalizeAccumulatorSource(target)}`];
...
function resolveSizeofValue(rawItem: string, ctx: TranspileContext): number | null {
  ...
  if (indexedItem) {
    ...
    if (object) return ctx.structDefs.get(object.structName)?.size ?? null;
    if (ctx.pointerStructTypes.has(name)) return ctx.pointerElementSizes.get(name) ?? null;
    const ramArray = ctx.localArrays.get(name);
    if (ramArray) return ramArray.elementSize;
    const xdataArray = ctx.xdataArrays.get(name);
    if (xdataArray) return xdataArray.elementSize;
    const codeArray = ctx.arrays.get(name);
    if (codeArray) return codeArray.elementSize;
    if (ctx.pointerVars.has(name)) return ctx.pointerElementSizes.get(name) ?? 1;
  }

  const access = resolveStructAccess(item, ctx, 0, false);
  if (access) return access.size;
...
function normalizedArrayOffset(index: number, array: LocalArrayVar): number {
  const normalized = ((index % array.elementCount) + array.elementCount) % array.elementCount;
  return normalized * array.elementSize;
}

function emitScaledIndexToA(index: string, elementSize: number, ctx: TranspileContext, line: number): string[] {
  const out = emitExprToA(index, ctx, line);
  if (elementSize === 2) out.push("rl a");
  else if (elementSize > 2) out.push("mov b,#" + toAsmByte8(elementSize), "mul ab");
  return out;
}

function emitPointerIndexLoad(name: string, index: string, word: boolean, ctx: TranspileContext, line: number): string[] {
  const pointer = resolveVariable(name, ctx);
  const kind = ctx.pointerVars.get(name);
  if (!pointer || !kind) return ["clr a", ...(word ? ["mov b,#0x0"] : [])];
  ...
  if (kind === "ram") {
    ...
    if (word) out.push("mov b,a", "inc r0", "mov a,@r0", "xch a,b");
    return out;
  }
  if (kind === "code") {
    const out = [`mov dpl,${variableTarget(pointer)}`, `mov dph,${variableTarget(pointer, 1)}`, ...emitScaledIndexToA(index, stride, ctx, line)];
    if (word) out.push("mov r1,a", "movc a,@a+dptr", "mov b,a", "mov a,r1", "inc a", "movc a,@a+dptr", "xch a,b");
    else out.push("movc a,@a+dptr");
    return out;
  }
  if (kind === "xdata") {
    const out = [
    ...
    if (word) out.push("mov b,a", "inc dptr", "movx a,@dptr", "xch a,b");
    return out;
  }
...
function emitBooleanExprToA(expr: string, ctx: TranspileContext, line: number): string[] {
...
function emitMulDivToA(left: string, op: string, right: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (op === "*") out.push("mul ab");
    ...
    if (op === "%") out.push("mov a,b");
    out.push(`${done}:`);
  }
  return out;
}

function emitShiftToA(left: string, op: string, right: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (op === ">>" && variable && variable.size >= 2) {
    if (amount >= variable.size * 8) return ["clr a"];
    ...
    for (let i = 0; i < amount; i++) out.push(`mov a,${variableTarget(high)}`, "clr c", "rrc a", `mov ${variableTarget(high)},a`, `mov a,${variableTarget(low)}`, "rrc a", `mov ${variableTarget(low)},a`);
    out.push(`mov a,${variableTarget(low)}`);
    return out;
  }
  if (amount >= 8) return ["clr a"];
  const out = emitExprToA(left, ctx, line);
  for (let i = 0; i < amount; i++) out.push("clr c", op === "<<" ? "rlc a" : "rrc a");
  return out;
}
...
function emitConditionFalseJump(cond: string, falseLabel: string, ctx: TranspileContext, line: number): string[] {
  ...
  if (cmp) {
    ...
    if (wideComparison) {
      return emitWideRelationalFalseJump(cmp.left, cmp.op as "<" | ">" | "<=" | ">=", cmp.right, signedComparison, falseLabel, ctx, line);
    }
    let out: string[];
    if (signedComparison) {
...
function emitWideRelationalFalseJump(
  leftExpr: string,
  op: "<" | ">" | "<=" | ">=",
  rightExpr: string,
  signed: boolean,
...
function emitWideEqualityFalseJump(
  left: ScalarVar,
  op: "==" | "!=",
  rightConstant: number | null,
  right: ScalarVar | null,
  falseLabel: string,
  ctx: TranspileContext,
): string[] {
  ...
  for (let byte = 0; byte < byteCount; byte++) {
    ...
    if (leftSource) out.push(`mov a,${leftSource}`);
    else out.push("clr a");
    if (rightSource) out.push(`cjne a,${rightSource},${mismatchLabel}`);
    else out.push(`cjne a,#${immediate ?? "0x0"},${mismatchLabel}`);
  }
...
function emitWordExpressionEqualityFalseJump(
  leftExpr: string,
  op: "==" | "!=",
  rightExpr: string,
  falseLabel: string,
  ...
  if (op === "==") out.push(`sjmp ${done}`, `${mismatch}:`, `ljmp ${falseLabel}`, `${done}:`);
  else out.push(`ljmp ${falseLabel}`, `${mismatch}:`);
  return out;
}

function emitConditionTrueJump(cond: string, trueLabel: string, ctx: TranspileContext, line: number): string[] {
  const text = trimOuter(cond.trim());
  const constant = tryEvalConst(text);
  if (constant != null) return constant ? [`sjmp ${trueLabel}`] : [];
  const bit = resolveVariable(text.toLowerCase(), ctx) ? undefined : ctx.sbitMap.get(text.toLowerCase());
  if (bit) return [`jb ${bit},${trueLabel}`];
...
function beginScopedDeclarationBindings(ctx: TranspileContext, declarators: string): () => void {
  ...
  const savedVars = new Map<string, ScalarVar | null>();
  const savedPointers = new Map<string, PointerKind | null>();
  const savedPointerSizes = new Map<string, number | null>();
  const savedPointerStructs = new Map<string, string | null>();
  const savedPointerConsts = new Map<string, boolean | null>();
  const savedArrays = new Map<string, LocalArrayVar | null>();
  const savedStructs = new Map<string, StructVar | null>();
...
function restoreMapBinding<T>(map: Map<string, T>, name: string, value: T | null): void {
  if (value == null) map.delete(name);
  else map.set(name, value);
}

function ensureVar(ctx: TranspileContext, name: string, line: number, size = 1): ScalarVar {
...
function variableTarget(variable: ScalarVar, byteOffset = 0): string {
  return toAsmByte(variable.addr + byteOffset);
}

function resolveVariable(name: string, ctx: TranspileContext): ScalarVar | null {
  const lowered = name.trim().toLowerCase();
  return ctx.vars.get(lowered) ?? ctx.globalVars.get(lowered) ?? null;
}

function resolveXdataVariable(name: string, ctx: TranspileContext): XdataVar | null {
  const lowered = name.trim().toLowerCase();
  return ctx.xdataVars.get(lowered) ?? ctx.globalXdataVars.get(lowered) ?? null;
}

function resolveXdataArrayAddress(expr: string, ctx: TranspileContext): number | null {
  const text = trimOuter(expr.trim());
  const direct = ctx.xdataArrays.get(text.toLowerCase());
  if (direct) return direct.baseAddr;
...
function resolveTarget(name: string, ctx: TranspileContext): string | null {
  ...
  if (/^0x[0-9a-f]+$/i.test(lowered)) return lowered;
  if (ctx.sfrMap.has(lowered)) return lowered;
  if (REGISTER_SET.has(lowered)) return lowered;
  return null;
}
...
function cUserLabel(ctx: TranspileContext, name: string): string {
...
function allocateXdataBlock(ctx: TranspileContext, length: number, line: number, label: string): number {
...
function allocateGlobalXdataBlock(nextXdataAddr: { value: number }, length: number, diagnostics: AsmDiagnostic[], label: string): number {
  const base = nextXdataAddr.value;
  nextXdataAddr.value += Math.max(1, length);
  if (nextXdataAddr.value > XDATA_LIMIT) diagnostics.push({ level: "error", message: `${label} exceeds ADuC841 XDATA 0x0000..0x07FF.` });
  return base;
}

function allocateFunctionParams(
  functions: Map<string, FunctionInfo>,
  structDefs: Map<string, StructDef>,
  nextVarAddr: { value: number },
  diagnostics: AsmDiagnostic[],
  ...
  for (const fn of functions.values()) {
    ...
    for (const param of fn.params) {
      ...
      nextVarAddr.value += size;
      offset += size;
      if (nextVarAddr.value > 0x80) diagnostics.push({ level: "error", message: `Parameters of ${fn.name} exceed internal RAM 0x30..0x7F.` });
    }
  }
}
  ...
  for (const param of fn.params) {
    ...
    if (param.pointer) {
      ...
      ctx.pointerElementSizes.set(
        param.name,
        param.pointeeSize ?? (param.structName ? Math.max(1, ctx.structDefs.get(param.structName)?.size ?? 1) : 1),
      );
...
function parseParamList(raw: string, diagnostics: AsmDiagnostic[], line: number): ParamInfo[] {
  const text = raw.trim();
  ...
  for (const part of splitCsv(text)) {
    ...
    if (pointer) {
      ...
      if (pointee?.unsupportedReason) diagnostics.push({ level: "error", line, message: `${pointer[2]}: ${pointee.unsupportedReason}` });
      params.push({
...
function splitStructMemberPath(path: string): string[] {
  const parts: string[] = [];
  let current = "";
  let bracketDepth = 0;
...
function resolveStructAccess(expr: string, ctx: TranspileContext, line: number, report = true): StructAccess | null {
  const text = trimOuter(expr.trim());
  let rootName = "";
  ...
  let structName = "";
  let storage: StructAccess["storage"] = "ram";
  let baseAddr: number | undefined;
  let pointer: ScalarVar | undefined;
  let isConst = false;
  const terms: StructAddressTerm[] = [];
  ...
  if (pointerRoot) {
    ...
    if (!structName || !kind || !pointer) return null;
    if (kind === "pdata" || kind === "far") {
      if (report) ctx.diagnostics.push({ level: "error", line, message: `Dereferencing a ${kind} structure pointer is not implemented.` });
      return null;
    }
    storage = kind === "code" ? "code" : kind === "xdata" ? "xdata" : "ram";
    isConst = ctx.pointerPointeeConst.get(rootName) === true || storage === "code";
    if (rootIndex) terms.push({ expression: rootIndex, stride: Math.max(1, ctx.pointerElementSizes.get(rootName) ?? 1) });
  } else {
    ...
    if (rootIndex) {
      if (object.elementCount <= 1) {
        if (report) ctx.diagnostics.push({ level: "error", line, message: `${rootName} is not an array of structures.` });
        return null;
      }
      const constant = tryEvalConst(rootIndex);
      if (constant != null) {
        if (constant < 0 || constant >= object.elementCount) {
          if (report) ctx.diagnostics.push({ level: "error", line, message: `Structure array index ${constant} is outside ${rootName}[0..${object.elementCount - 1}].` });
          return null;
        }
        constantOffset += constant * def.size;
      } else terms.push({ expression: rootIndex, stride: def.size });
    } else if (object.elementCount > 1) {
      if (report) ctx.diagnostics.push({ level: "error", line, message: `Structure array ${rootName} requires an index before member access.` });
      return null;
    }
  }
  ...
  for (let partIndex = 0; partIndex < parts.length; partIndex++) {
    ...
    if (!field) {
      if (report) ctx.diagnostics.push({ level: "error", line, message: `Struct ${current.name} has no field named ${segment[1]}.` });
      return null;
    }
    ...
    if (fieldIndex) {
      if (field.elementCount <= 1) {
        if (report) ctx.diagnostics.push({ level: "error", line, message: `Field ${current.name}.${field.name} is not an array.` });
        return null;
      }
      const constant = tryEvalConst(fieldIndex);
      if (constant != null) {
        if (constant < 0 || constant >= field.elementCount) {
          if (report) ctx.diagnostics.push({ level: "error", line, message: `Field index ${constant} is outside ${current.name}.${field.name}[0..${field.elementCount - 1}].` });
          return null;
        }
        constantOffset += constant * field.elementSize;
      } else terms.push({ expression: fieldIndex, stride: field.elementSize });
    } else if (field.elementCount > 1 && partIndex < parts.length - 1) {
      if (report) ctx.diagnostics.push({ level: "error", line, message: `Array field ${current.name}.${field.name} requires an index.` });
      return null;
    }
    ...
    if (!field.structName) {
      if (report) ctx.diagnostics.push({ level: "error", line, message: `Scalar field ${current.name}.${field.name} has no nested members.` });
      return null;
    }
...
function emitScaledIndexToWord(index: string, stride: number, ctx: TranspileContext, line: number): string[] {
  ...
  if (stride <= 0xff) {
    const out = emitExprToA(index, ctx, line);
    if (stride === 1) return [...out, "mov b,#0x0"];
...
function emitStructAddressToWord(access: StructAccess, ctx: TranspileContext, line: number): string[] {
  ...
  if (access.baseAddr != null) {
    ...
    if (access.pointer.size >= 2) out.push(`mov a,${variableTarget(access.pointer, 1)}`, `mov ${variableTarget(addrHigh)},a`);
    else out.push(`mov ${variableTarget(addrHigh)},#0x0`);
  } else return ["clr a", "mov b,#0x0"];

  for (const term of access.terms) {
    out.push(
      ...emitScaledIndexToWord(term.expression, term.stride, ctx, line),
      `mov ${variableTarget(prodLow)},a`,
      `mov ${variableTarget(prodHigh)},b`,
      `mov a,${variableTarget(addrLow)}`,
      `add a,${variableTarget(prodLow)}`,
      `mov ${variableTarget(addrLow)},a`,
      `mov a,${variableTarget(addrHigh)}`,
      `addc a,${variableTarget(prodHigh)}`,
  ...
  if (access.constantOffset) {
    out.push(
      `mov a,${variableTarget(addrLow)}`,
      `add a,#${toAsmByte8(access.constantOffset)}`,
      `mov ${variableTarget(addrLow)},a`,
      `mov a,${variableTarget(addrHigh)}`,
      `addc a,#${toAsmByte8(access.constantOffset >> 8)}`,
      `mov ${variableTarget(addrHigh)},a`,
    );
  }
...
function emitLoadStructAccess(access: StructAccess, ctx: TranspileContext, line: number, asWord: boolean): string[] {
  if (access.isArray) {
  ...
  if (access.baseAddr != null && access.terms.length === 0) {
    ...
    if (access.storage === "ram") {
      if (width >= 2) return [`mov a,${toAsmByte(address)}`, `mov b,${toAsmByte(address + 1)}`];
      const out = [`mov a,${toAsmByte(address)}`];
      return asWord ? [...out, ...emitSignOrZeroExtendA(access.signed, ctx)] : out;
    }
    if (access.storage === "xdata") {
      if (width >= 2) return emitLoadWordFromXdataAddress(address);
  ...
  if (access.storage === "ram") {
    ...
    if (width >= 2) out.push("mov b,a", "inc r0", "mov a,@r0", "xch a,b");
    else if (asWord) out.push(...emitSignOrZeroExtendA(access.signed, ctx));
    return out;
  }
  out.push("mov dpl,a", "mov dph,b");
  if (access.storage === "xdata") {
    out.push("movx a,@dptr");
    if (width >= 2) out.push("mov b,a", "inc dptr", "movx a,@dptr", "xch a,b");
    else if (asWord) out.push(...emitSignOrZeroExtendA(access.signed, ctx));
    return out;
  }
  out.push("clr a", "movc a,@a+dptr");
  if (width >= 2) out.push("mov b,a", "inc dptr", "clr a", "movc a,@a+dptr", "xch a,b");
  else if (asWord) out.push(...emitSignOrZeroExtendA(access.signed, ctx));
  return out;
}

function emitSignOrZeroExtendA(signed: boolean, ctx: TranspileContext): string[] {
  if (!signed) return ["mov b,#0x0"];
...
function emitStoreStructAccess(access: StructAccess, rhs: string, ctx: TranspileContext, line: number): string[] {
  if (access.isArray) {
  ...
  if (access.baseAddr != null && access.terms.length === 0) {
    const address = access.baseAddr + access.constantOffset;
    if (access.storage === "ram") {
      if (word) return [...emitExprToWord(rhs, ctx, line), `mov ${toAsmByte(address)},a`, `mov ${toAsmByte(address + 1)},b`];
      return emitAssignToTarget(toAsmByte(address), rhs, ctx, line);
    }
    if (word) return [...emitExprToWord(rhs, ctx, line), ...emitStoreWordToXdataAddress(address)];
  ...
  if (access.storage === "ram") out.push("mov r0,a");
  else out.push("mov dpl,a", "mov dph,b");
  if (word) out.push("pop b", "pop acc");
  else out.push("pop acc");
  if (access.storage === "ram") {
    out.push("mov @r0,a");
    if (word) out.push("inc r0", "mov a,b", "mov @r0,a");
  } else {
    out.push("movx @dptr,a");
    if (word) out.push("inc dptr", "mov a,b", "movx @dptr,a");
  }
  return out;
}
...
function resolveAddressExpression(expr: string, ctx: TranspileContext): number | null {
  ...
  if (xdataVariable) return xdataVariable.addr;
  ...
  if (structVar && structVar.elementCount === 1) return structVar.baseAddr;
  ...
  if (structElement) {
    ...
    if (object && def && index != null && index >= 0 && index < object.elementCount) return object.baseAddr + index * def.size;
  }
  const access = resolveStructAccess(text, ctx, 0, false);
  if (access?.baseAddr != null && access.terms.length === 0) return access.baseAddr + access.constantOffset;
  const xdataArr = ctx.xdataArrays.get(text.toLowerCase());
  if (xdataArr) return xdataArr.baseAddr;
  ...
  if (idx) {
    ...
    if (xdata && constIndex != null) return xdata.baseAddr + normalizedArrayOffset(constIndex, xdata);
    if (local && constIndex != null) return local.baseAddr + normalizedArrayOffset(constIndex, local);
  }
  return null;
}
...
function emitStoreToLValue(lhs: string, rhs: string, ctx: TranspileContext, line: number): string[] | null {
  ...
  if (directXdata) return emitAssignToXdataVariable(directXdata, rhs, ctx, line);

  const structAccess = resolveStructAccess(lhs, ctx, line, true);
  if (structAccess) return emitStoreStructAccess(structAccess, rhs, ctx, line);
  ...
  if (localArr) {
    ...
    if (xdataArr) return emitXdataArrayStore(xdataArr, localArr[2], rhs, ctx, line);
    ...
    if (arr) {
      ...
      if (idxConst != null) {
        ...
        if (arr.elementSize >= 2) return [...emitExprToWord(rhs, ctx, line), `mov ${toAsmByte(address)},a`, `mov ${toAsmByte(address + 1)},b`];
        return emitAssignToTarget(toAsmByte(address), rhs, ctx, line);
      }
    ...
    if (pointer && kind) {
      ...
      if (kind === "ram") out.push(`mov r1,${variableTarget(pointer)}`, "add a,r1", "mov r0,a");
      else out.push(`mov r1,${variableTarget(pointer)}`, "add a,r1", "mov dpl,a", "clr a", `mov r1,${variableTarget(pointer, 1)}`, "addc a,r1", "mov dph,a");
      if (word) out.push("pop b", "pop acc");
      else out.push("pop acc");
      if (kind === "ram") {
        out.push("mov @r0,a");
...
function transpileSwitchStatement(
  ...
  parsed.forEach((item, index) => {
    if (item.kind !== "case" || item.value == null || !item.dispatchable) return;
...
function findSwitchMarkers(body: string, baseLine: number, diagnostics: AsmDiagnostic[]): SwitchMarker[] {
...
function findSwitchCaseColon(body: string, start: number, mask: boolean[]): number {
  let paren = 0;
  let bracket = 0;
  let ternary = 0;
  for (let index = start; index < body.length; index++) {
    ...
    else if (paren === 0 && bracket === 0 && ch === "?") ternary++;
...
function switchExpressionSize(expression: string, ctx: TranspileContext): number {
  const text = trimOuter(expression).toLowerCase();
  const variable = resolveVariable(text, ctx);
  if (variable) return variable.size;
  const xdataVariable = resolveXdataVariable(text, ctx);
  if (xdataVariable) return xdataVariable.size;
  if (ctx.sfr16Map.has(text)) return 2;
  const cast = peelLeadingScalarCasts(expression);
  if (cast.type) return cast.type.size;
...
function walkStructPath(structName: string, path: string, ctx: TranspileContext): { offset: number; size: number } | null {
  let currentStruct: StructDef | undefined = ctx.structDefs.get(structName);
  ...
  for (let idx = 0; idx < parts.length; idx++) {
    ...
    if (!match) return null;
    const field: StructField | undefined = currentStruct.fields.find((item) => item.name === match[1].toLowerCase());
    ...
    const nested: StructDef | undefined = field.structName ? ctx.structDefs.get(field.structName) : undefined;
    if (!nested) return null;
...
function pointerImmediate(expr: string, kind: PointerKind, ctx: TranspileContext): { low: string; high: string } | null {
  const text = trimOuter(expr.trim());
  if (kind === "code") {
  ...
  if (numeric != null) return { low: toAsmByte8(numeric), high: toAsmByte8(numeric >> 8) };
  const address = resolveAddressExpression(text.replace(/^&\s*/, ""), ctx);
  if (address != null) return { low: toAsmByte8(address), high: toAsmByte8(address >> 8) };
  return null;
}

function emitAddressExpressionToWord(expr: string, ctx: TranspileContext, line: number): string[] | null {
  const text = trimOuter(expr.trim());
  const address = resolveAddressExpression(text, ctx);
  if (address != null) return [`mov a,#${toAsmByte8(address)}`, `mov b,#${toAsmByte8(address >> 8)}`];

  const structAccess = resolveStructAccess(text, ctx, line, false);
  if (structAccess) return emitStructAddressToWord(structAccess, ctx, line);
  ...
  if (structElement) {
    ...
    if (object && def) {
      ...
      if (constant != null) access.constantOffset = constant * def.size;
      else access.terms.push({ expression: structElement[2], stride: def.size });
  ...
  if (indexed) {
    const array = ctx.xdataArrays.get(indexed[1].toLowerCase());
    if (array) return [...emitXdataArrayAddress(array, indexed[2], ctx, line), "mov a,dpl", "mov b,dph"];
...
function emitLoadByteFromXdataAddress(address: number): string[] {
  return [`mov dptr,#${toAsmByte(address)}`, "movx a,@dptr"];
}

function emitLoadWordFromXdataAddress(address: number): string[] {
  return [`mov dptr,#${toAsmByte(address)}`, "movx a,@dptr", "mov b,a", "inc dptr", "movx a,@dptr", "xch a,b"];
}

function emitStoreByteToXdataAddress(address: number): string[] {
  return [`mov dptr,#${toAsmByte(address)}`, "movx @dptr,a"];
}

function emitStoreWordToXdataAddress(address: number): string[] {
...
function emitXdataConstantBytes(baseAddr: number, bytes: number[]): string[] {
  ...
  bytes.forEach((value, index) => {
    ...
    if (index + 1 < bytes.length) out.push("inc dptr");
  });
  return out;
}

function emitXdataArrayAddress(array: XdataArrayVar, index: string, ctx: TranspileContext, line: number): string[] {
  const constant = tryEvalConst(index);
  if (constant != null) return [`mov dptr,#${toAsmByte(array.baseAddr + normalizedArrayOffset(constant, array))}`];
  return [
...
function emitXdataArrayLoad(array: XdataArrayVar, index: string, word: boolean, ctx: TranspileContext, line: number): string[] {
  const out = [...emitXdataArrayAddress(array, index, ctx, line), "movx a,@dptr"];
  if (word) out.push("mov b,a", "inc dptr", "movx a,@dptr", "xch a,b");
  return out;
}

function emitXdataArrayStore(array: XdataArrayVar, index: string, rhs: string, ctx: TranspileContext, line: number): string[] {
...
function unsupportedArithmeticLiteral(expression: string): string | null {
...
function tryEvalConst(raw: string): number | null {
  ...
  if (cast.type?.unsupportedReason) return null;
  ...
  if (/(?:\+\+|--|=>|===|!==|\b(?:new|this)\b)/.test(expr)) return null;
  try {
...
function decodeCCharacter(token: string): number | null {
  const body = token.slice(1, -1);
  if (!body) return null;
  if (!body.startsWith("\\")) return body.charCodeAt(body.length - 1) & 0xff;
  const esc = body.slice(1);
  const simple: Record<string, number> = {
    "0": 0, "a": 7, "b": 8, "t": 9, "n": 10, "v": 11, "f": 12, "r": 13,
    "\\": 92, "'": 39, '"': 34, "?": 63,
  };
  if (Object.prototype.hasOwnProperty.call(simple, esc)) return simple[esc];
  if (/^x[0-9a-fA-F]+$/.test(esc)) return Number.parseInt(esc.slice(1), 16) & 0xff;
  if (/^[0-7]{1,3}$/.test(esc)) return Number.parseInt(esc, 8) & 0xff;
  return null;
}

function formatBuiltinBitOperand(name: string, address: number): string {
  const register = Object.entries(ADUC841_SFR).find(([candidate, base]) =>
    /^(?:p[0-3]|acc|b)$/.test(candidate) && address >= base && address < base + 8
  );
...
function parseSwitchStatementText(raw: string): { expression: string; body: string } | null {
  const start = skipWhitespace(raw, 0);
  if (raw.slice(start, start + 6).toLowerCase() !== "switch") return null;
  const before = start > 0 ? raw[start - 1] : "";
  const after = raw[start + 6] ?? "";
  if ((before && /[A-Za-z0-9_]/.test(before)) || (after && /[A-Za-z0-9_]/.test(after))) return null;
  ...
  if (bodyEnd < 0 || raw.slice(bodyEnd + 1).trim()) return null;
...
function findTopLevelBinaryOperator(text: string, operators: string[]): { left: string; op: string; right: string } | null {
  ...
  for (let i = text.length - 1; i >= 0; i--) {
    ...
    if (depthParen || depthBracket || depthBrace) continue;
    for (const op of sorted) {
      const start = i - op.length + 1;
      if (start < 0 || text.slice(start, i + 1) !== op) continue;
      if ((op === "+" || op === "-") && (start === 0 || /[+\-*/%&|^!<>=?:,(]/.test(text[start - 1]))) continue;
      if (op === "&" && (text[start - 1] === "&" || text[i + 1] === "&")) continue;
      if (op === "|" && (text[start - 1] === "|" || text[i + 1] === "|")) continue;
      if (op === "<" && (text[start - 1] === "<" || text[i + 1] === "<" || text[i + 1] === "=")) continue;
      if (op === ">" && (text[start - 1] === ">" || text[i + 1] === ">" || text[i + 1] === "=")) continue;
...
function findTopLevelTernary(text: string): { condition: string; whenTrue: string; whenFalse: string } | null {
  let depthParen = 0;
  let depthBracket = 0;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === "?") {
      ...
      if (nestedQuestions > 0) { nestedQuestions--; continue; }
...
function findComparator(text: string): { left: string; op: string; right: string } | null {
  return findTopLevelBinaryOperator(text, ["<=", ">=", "==", "!=", "<", ">"]) as { left: string; op: string; right: string } | null;
}
...
function findMatchingParenLike(source: string, openIndex: number, openChar: string, closeChar: string): number {
  ...
  for (let i = openIndex; i < source.length; i++) {
    ...
    if (ch === "/" && next === "/") { i++; lineComment = true; continue; }
    if (ch === "/" && next === "*") { i++; blockComment = true; continue; }
    if (ch === '"' || ch === "'") { quote = ch; continue; }
