...
export function analyzeSource(mode: SourceMode, code: string): DiagnosticItem[] {
  switch (mode) {
    case "asm":
...
function analyzeAsm(code: string): DiagnosticItem[] {
...
function analyzeC(code: string): DiagnosticItem[] {
  const diagnostics: DiagnosticItem[] = [];
  const lines = code.split(/\r?\n/);
  let braceBalance = 0;
...
function analyzeHex(code: string): DiagnosticItem[] {
  const diagnostics: DiagnosticItem[] = [];
  const lines = code
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function analyzeJs(code: string): DiagnosticItem[] {
  try {
