...
export const exampleScripts: ScriptExample[] = [
  {
    ...
    description:
      "Пише в latch 0x07 і біжить активним бітом по LED-лінійці в active-low режимі.",
...
`,
  },
  {
    ...
    description:
      "Записує 1-9-8-8 у чотири цифри семисегментника через адреси 0x01..0x04.",
    code: `export async function main({ board }) {
  const digits = [
...
`,
  },
  {
    ...
    description:
      "Сканує 5x7 матрицю через 0x05/0x06 і тримає видиму букву S через мультиплексування.",
    code: `export async function main({ board }) {
  const frames = [
...
`,
  },
  {
    ...
    description:
      "Сканує клавіатуру через 0x60 / 0x50 / 0x30 і підсвічує окремий LED під натиснуту кнопку.",
    code: `export async function main({ board }) {
...
`,
  },
  {
    ...
    description:
      "Простий high-level приклад для LCD: очищення, два рядки тексту і курсор у різні позиції.",
...
`,
  },
  {
    ...
    description:
      "Читає X/Y джойстика і малює точку на 5x7 матриці як візуалізацію АЦП.",
    code: `export async function main({ devices }) {
  while (true) {
...
`,
  },
  {
    ...
    description:
      "High-level демо: швидкість двигуна залежить від X-положення джойстика.",
    code: `export async function main({ devices }) {
  while (true) {
...
`,
  },
  {
    ...
    description:
      "High-level демо: прямокутний аудіосигнал із паузою між каналами.",
    code: `export async function main({ devices }) {
  let tick = 0;
  while (true) {
